...
"""Parse the shipped d3 XZZX ``circuit_ideal.stim`` into the QutritDM Schedule.
...
(:class:`qec_twin.forward.exact.qutrit_dm.QutritDM`) evolves only the DATA register
...
Two contract-mandated normalizations are applied AT PARSE TIME (§1.1/§1.2):
  ...
  * **surplus qubits dropped.** The shipped *multi-round* circuit declares 20 qubit
...
class RoundDataFrame:
    ...
    The shipped d3 XZZX circuit applies, on the DATA qubits, single-qutrit gates that are
...
def _per_round_data_frame(
    ...
    across all shipped d3 patches/bases.
    """
    import stim
...
def parse_xzzx_circuit(
    ...
    *,
    ...
    """Parse a shipped d3 XZZX ``circuit_ideal.stim`` into an :class:`XZZXSchedule`.
...
# --------------------------------------------------------------------------- #
# Convenience: locate the shipped d3_at_q6_7 r01 patch                         #
# --------------------------------------------------------------------------- #
...
def default_r01_paths(patch: str = "d3_at_q6_7", basis: str = "X") -> tuple[Path, Path]:
    """The (circuit_ideal.stim, metadata.json) paths for a shipped r01 d3 patch.

    r01 is the R=1 deliverable instance: natively 17 qubits / 8 detectors, no surplus.
    """
    base = DEFAULT_DATASET_ROOT / patch / basis / "r01"
    return base / "circuit_ideal.stim", base / "metadata.json"


def default_r10_paths(patch: str = "d3_at_q6_7", basis: str = "X") -> tuple[Path, Path]:
    """The (circuit_ideal.stim, metadata.json) paths for a shipped r10 d3 patch.
    ...
    """
    base = DEFAULT_DATASET_ROOT / patch / basis / "r10"
    return base / "circuit_ideal.stim", base / "metadata.json"
