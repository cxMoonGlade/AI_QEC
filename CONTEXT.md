...
> **⚑ Reframe (ADR 0009).** Orbit-symmetry compression as an identifiability or
> parameter-economy lever is **retired** — the B-path broke the observational
> alias with **probe richness (data)**, not parameter-tying. The catalog symbol
> `omega(j)` survives only as **known teacher structure** (used silently for
> compute/correctness), never as a thesis. Package: `qec_twin`. Spec: `docs/TWIN.md`.
...
substrate (the twin's channel substrate, `qec_twin.primitives.diff_cptp_channel`,
