...
| leakage target | `WG_L1=5e-3/cycle` | `paper-measured` target | Miao et al., Nature Physics 19 (2023), Fig. 3c, estimated leakage generated per cycle `~5e-3` | target scale for the Miao device/protocol |
| resolved exchange angle | `theta≈0.102444 rad` | `calibrated-to-paper` | project bisection through the exact project WG channel to hit `WG_L1=5e-3`; the paper did not measure this angle | project channel parameter calibrated to the target |
| seepage coordinate | `g_seep=0.09` | `project-design` with literature-scale anchor | McEwen et al., Nature Communications 12 (2021), Table S1: no-reset data `gamma_down=9.1%/round`; approximate mapping into a different project channel/device, not a fit | cross-paper scale comparator, not a calibration |
| heating coordinate | `g_heat=0` | `project-design` | incoherent-heating-off ablation/default | declared simplification |
| leaked-readout bias | `b=0.9` | `project-design`, unsupported | no magnitude measurement found; `qutrit_teachers.py:50-55,116-135` instead requires a nuisance sweep `b in [0.5,1]` and explicitly rejects an invented `0.9` pin | **must not support a physical/realistic claim** |
...
using `b=0.9` can be promoted to a hardware prediction.

The two direct primary anchors support only these atomic statements:

1. Miao et al. measured an approximately `5e-3/cycle` leakage-source scale in their DQLR surface-code
   experiment (Fig. 3c).
...
|---|---|---|---|
| Axis-1 G2 demonstration | `zeta/2pi=370 kHz`, `T1=T2=30 us`, drive `pi/25 ns`, registered `dt`/effect bands | `project-design` | formal BCH/channel implementation fixture; explicitly not an idle-Google calibration |
| Axis-1 primitive defaults | e.g. readout dephasing `1e-3/ns`, zero upward rate, zero residual fSim terms | `project-design` / fail-inert defaults | class-(c) lowering inputs, not measured primitive rates |
...
| shared-source coupling defaults | representative 6/6.1 GHz qubits, `alpha=-300 MHz`, 25 ns gate, base phase `1.6e-4`, `T_phi=75 us`, SPAM/depolarization bases and sensitivities | `project-design` / `convenience-default` | formulas may be literature-derived; these constants are not device calibration and “Calibration” in a class name does not change that |
...
| phase burst | `-2 MHz`, `1 ms`, `T1=10 us`, echo factor `0.05`, default event probability `0` | mixed: one-paper scale anchors plus `project-design` mapping | Kurilovich-like scale bracket; the multiplicative `0.05` and complete tuple are not directly measured as implemented |
...
| public MCWF/MPS execution defaults | microstep count `1`, trajectory count `1`, leaked-readout `b=1`, mass-residual budget `.1` | `convenience-default` / `project-design` | routing/smoke defaults only; one trajectory is not distribution evidence and `b=1` is not calibrated |
...
| process infidelity strict `1e-6`, gross `0.1`; record-TV strict `1e-6`, gross `0.2`, ceiling `0.45`; confidence `0.999` | dense MCWF tripwires | `project-design` / `numerical-only` | implementation routing and no-op rejection only; a gross pass is not physical faithfulness |
| `p_ro=.01`, `p_reset=.005`, drift `s<=.3`, `Z=3`, `N<=1e6` | notion-2 Class-1 go/no-go | `project-design` / `convenience-default` | conditional bandwidth calculation; `s<=.3` is not a hardware-calibrated realistic ceiling |
| generic anchor bands such as `6/sqrt(N)`, `8/sqrt(N)`, scalar `1e-9` | certification heuristics | `project-design` / `numerical-only` | declared project gates, not coverage theorems |
...
## 9. Source-code correction queue (completed for the active registered-preset path, 2026-07-13)
