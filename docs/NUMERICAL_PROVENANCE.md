...
| Restricted MPS Record layout and schemas | `axis1_schedule_record_layout.v1`; direct QT/MPS and MCWF/MPS v6; QT aggregate/resource manifests v4; Carrier wrapper/auto route v3 | `interface-identity` | Immutable keys, targets, bases, per-target reset masks, slices, and XOR columns are parsed from the compiler-sealed schedule before execution. Current versions also bind sampled-support, transitive child authentication, exact-shape resource evidence, and fail-closed verdict semantics. They are implementation/schema facts, not sampled values, calibration, or independent oracle truth; no earlier-version fallback is retained |
