...
|---|---|---|---|---|
| DM-PEPO carrier, single-layer Tr(ρΠ) | arXiv:1607.06460 (Darmawan–Poulin; >100 data qubits — 153q per the DESIGN correction block + survey map — single-round, χ_b=8) *(foundational, pre-2022)* | same (syndrome Born probs from PEPO contraction) | `darmawan_poulin_realistic_noise_1607.06460.md` (+ DESIGN §correction for the 153q figure) | — (new build) |
...
| S8 | float64/complex128 everywhere | (c) hard rule | the float32 `1−ε == 1.0f` trap; no fp32 in any tail/gap/ledger quantity (Rudolph–Tindall's fp32 GPU numbers do not transfer as-is) |
