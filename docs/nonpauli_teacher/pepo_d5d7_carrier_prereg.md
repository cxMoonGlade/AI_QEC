...
| S9 | **Truncated-ρ positivity is NOT enforced by construction** (v2 NEW — the BLOCKER fix) | (b) banded + (c) STOP rule | Ledger row C3 (band + witness re-based v2.4 on the G1.9-pre measurement — see C3 for the formulas; the earlier 1e-6/1e-8/1e-4 constants superseded pre-build); demonstrated-to-trip variant required; REGISTERED ESCALATION: switch the carrier to the locally-purified LPDO form ρ = XX† (Werner 1412.5746 — positivity by construction, at higher bond cost) if the witness trips persistently |
