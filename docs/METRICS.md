...
| MCWF frozen-dynamics artifact reference gate | Compile each production term once; build connected Hamiltonian-group gates from those frozen terms; then independently reconstruct every term, group partition/support/order, and group gate from certifier-local NumPy formulas and SciPy `expm`. Terms require exact declared structural zeros and floating difference at most `NUMERICAL_ZERO`; group gates require `max_ij abs(U_frozen[i,j]-U_reference[i,j]) <= 1000*NUMERICAL_ZERO`. The packet must cover every substep/term/group, bind Carrier-program/artifact hashes, the reference/certifier/carrier-operator sources, and the transitive ideal-control-generator and selection-family sources, plus local dimensions and finite-step controls, and pass a post-execution artifact-content recheck. The policy, Carrier, and auto seams require the current authenticated packet. This is a non-metric class-(a/c) software artifact/integrity gate, not state or Record distance, a physical tolerance, source closure, calibration, or production-scalability evidence. | `certify.axis1_mps.mcwf_dynamics_artifact_reference_certification`, `validate_mcwf_dynamics_artifact_reference_certification`, `_mcwf_dynamics_artifact_reference_failure`; TOCTOU, stale-transitive-source, state-insensitive grouping, structural-zero, packet-forgery, and public transitive-authentication falsifiers in `tests/test_axis1_mcwf_dense_certification.py`, `tests/test_mcwf_operator_reference.py`, and `tests/test_mps_carrier_child_authentication.py` |
| Isolated QuTiP vs public MCWF X/Z statistical differential | For each empirical pair, `TV=0.5*sum_x abs(phat_Qutip(x)-phat_project(x))`. With `n_Qutip=n_project=2048`, overall `alpha=0.01`, and six simultaneous comparisons, each comparison receives `alpha_j=alpha/6`; its two independent sample radii are `r(n,k,alpha_j/2)=min(1,sqrt(log((2^k-2)/(alpha_j/2))/(2n)))`, and passes only when `TV <= r_left+r_right`. Three joint ordered-Record views use `k=16`; three directed X-after marginals use `k=2`. The frozen fixture sets `s=exp(-gamma*t)=0.25`, so the registered `sqrt(s) -> s` coherence mutation has marginal TV `0.125`, above the directed simultaneous radius (about `0.08718`), and must fail; a deterministic final-Z-bit flip is a second verdict-driving negative control. Reused qubit samples make the six views correlated, not six independent experiments; the six-way alpha allocation is conservative. The worker v2 payload is strict-JSON parsed, recursively exact-field checked, and independently recomputed before it is retained unchanged in a raw-byte-bound transport envelope. This is class-(c) finite-sample evidence for one two-qubit, two-boundary T1 fixture, not complete QEC Record, qutrit/leakage, trajectory coupling, scalability, or production evidence. | `scripts/external_baselines/qutip_mcwf_xz_protocol.py`, `qutip_mcwf_xz_worker.py`, and `run_qutip_mcwf_xz_comparison.py`; `tests/test_external_qutip_mcwf_xz_comparison.py` |
