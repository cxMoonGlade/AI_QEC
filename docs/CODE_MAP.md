...
<!-- code-map-inputs-sha256: 9a0e2955e4c7448e045d1ff65511020bc2a78e00a7e23c4f9f531e191c67879e -->
- **code-map inputs sha256:** `9a0e2955e4c7448e045d1ff65511020bc2a78e00a7e23c4f9f531e191c67879e`  •  **git HEAD:** `216d71a`
...
- ✅ canonical ownership clean — installed modules and all 72 declared acceptance files have no executable retired-namespace import, dynamic-import, or patch-target edge.
...
- ✅ acceptance execution lanes — default `cpu_light`; `cpu_exclusive`=7, `cpu_light`=30, `gpu_serial`=35. Every acceptance file resolves to exactly one validated lane.
...
- **`numerics.py`** (222 LOC) — support: `shared_numerical_policy` — (no docstring)
...
- **`interop.py`** (349 LOC) — service: `pymatching_decoder`, `record_io_and_dem_reduction` — Correlated-Pauli DEM reduction of emitted ``{det,obs}`` records.
...
- **`noise_spec.py`** (1192 LOC) — service: `frontend_compile_and_records`, `source_stim_pauli_projection` — Noise specifications for the simulator frontend.
...
- **`coupling.py`** (862 LOC) — service: `classical_finite_rtn_source_chain` — Axis-2 shared-source parameter fan-out for coupled noise processes.
...
- **`process.py`** (1098 LOC) — service: `classical_burst_storm_sources`, `classical_finite_rtn_source_chain` — Axis-2 source processes for cross-cycle coupled noise processes.
