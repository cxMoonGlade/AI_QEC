...
<!-- code-map-inputs-sha256: e2e7363a87bf1bd45db427aabafb27cca032baf2066c99e181bdb6594a0f07cd -->
- **code-map inputs sha256:** `e2e7363a87bf1bd45db427aabafb27cca032baf2066c99e181bdb6594a0f07cd`  •  **git HEAD:** `38ae4c3`
...
- ✅ canonical ownership clean — installed modules and all 102 declared acceptance files have no executable retired-namespace import, dynamic-import, or patch-target edge.
...
- ✅ acceptance execution lanes — default `cpu_light`; `cpu_exclusive`=7, `cpu_light`=52, `gpu_serial`=43. Every acceptance file resolves to exactly one validated lane.
...
- **`axis1_carrier_execution.py`** (6408 LOC) — service: `axis1_dense_jointl_record`, `restricted_axis1_1d_mps` — Executable Axis-1 carrier seam.
...
---
_display inventory (excludes `__init__.py`): 17 packages, 111 modules, 171 public classes, 651 public functions (committed `src/` + dev `tools/`) + curated local-only clusters above. The release-contract count above separately includes all 111 installed package modules, including namespace facades._
