...
<!-- code-map-inputs-sha256: 86900c16bb8f67a240417cfb6b4f0e3b27f839cc05cb3e6c007338a42d33d6b5 -->
- **code-map inputs sha256:** `86900c16bb8f67a240417cfb6b4f0e3b27f839cc05cb3e6c007338a42d33d6b5`  •  **git HEAD:** `4e04435`
...
- ✅ reverse module coverage clean — all 104 installed Python modules are classified (79 service implementation modules + 25 explicit support modules).
- ✅ canonical ownership clean — installed modules and all 71 declared acceptance files have no executable retired-namespace import, dynamic-import, or patch-target edge.
...
- ✅ acceptance execution lanes — default `cpu_light`; `cpu_exclusive`=7, `cpu_light`=29, `gpu_serial`=35. Every acceptance file resolves to exactly one validated lane.
...
| **`shared_numerical_policy`** | `src/error_coupling_simulator/numerics.py` | `peps_single_wire`<br>`classical_finite_rtn_source_chain` | Shared numerical-zero/probability-floor constants; not a physical mechanism, metric, or decoder-floor service. |
...
- **`within_cycle.py`** (1099 LOC) — service: `fused_within_cycle` — Package-local within-cycle qutrit schedule and WG leakage host.
...
- **`fet.py`** (456 LOC) — service: `peps_single_wire` — Environment-aware single-bond rank selection for the single-wire PEPS carrier.
...
- **`trajectory.py`** (804 LOC) — service: `peps_single_wire` — Per-shot single-wire PEPS trajectory loop.
...
- **`axis1_mcwf_dense_certification.py`** (1351 LOC) — support: `axis1_certification_harnesses` — Dense-reference certification for the restricted MCWF/MPS carrier.
...
- **`axis1_mcwf_mps_execution.py`** (2333 LOC) — service: `restricted_axis1_1d_mps` — Fixed-microstep MCWF-over-MPS execution for Axis-1 carrier programs.
...
- **`interop.py`** (324 LOC) — service: `pymatching_decoder`, `record_io_and_dem_reduction` — Correlated-Pauli DEM reduction of emitted ``{det,obs}`` records.
...
- **`qutrit_leakage.py`** (477 LOC) — service: `qutrit_leakage_channel_and_process` — Wood--Gambetta qutrit leakage channels and specified noise processes.
...
- **`coupling.py`** (574 LOC) — service: `classical_finite_rtn_source_chain` — Axis-2 shared-source parameter fan-out for coupled noise processes.
...
- **`process.py`** (1026 LOC) — service: `classical_burst_storm_sources`, `classical_finite_rtn_source_chain` — Axis-2 source processes for cross-cycle coupled noise processes.
...
---
_display inventory (excludes `__init__.py`): 15 packages, 100 modules, 159 public classes, 430 public functions (committed `src/` + dev `tools/`) + curated local-only clusters above. The release-contract count above separately includes all 104 installed package modules, including namespace facades._
