...
<!-- code-map-inputs-sha256: 524a8e3dbad497eff10abbee20eba2c0ba01b655f2c178939582b3dfdf121321 -->
- **code-map inputs sha256:** `524a8e3dbad497eff10abbee20eba2c0ba01b655f2c178939582b3dfdf121321`  •  **git HEAD:** `1fd2813`
...
- ✅ reverse module coverage clean — all 111 installed Python modules are classified (87 service implementation modules + 24 explicit support modules).
- ✅ canonical ownership clean — installed modules and all 100 declared acceptance files have no executable retired-namespace import, dynamic-import, or patch-target edge.
...
- ✅ acceptance execution lanes — default `cpu_light`; `cpu_exclusive`=7, `cpu_light`=51, `gpu_serial`=42. Every acceptance file resolves to exactly one validated lane.
...
- **`axis1_mps.py`** (4512 LOC) — service: `restricted_axis1_1d_mps` — Evaluator-side certification for restricted MCWF/MPS execution evidence.
...
- **`axis1_carrier_execution.py`** (3984 LOC) — service: `axis1_dense_jointl_record` — Executable Axis-1 carrier seam.
...
- **`axis1_mcwf_mps_execution.py`** (3340 LOC) — service: `restricted_axis1_1d_mps` — Fixed-microstep MCWF-over-MPS execution for Axis-1 carrier programs.
...
- **`mutation.py`** (4282 LOC) — Registry-driven L2 mutation runner.
...
---
_display inventory (excludes `__init__.py`): 17 packages, 111 modules, 170 public classes, 649 public functions (committed `src/` + dev `tools/`) + curated local-only clusters above. The release-contract count above separately includes all 111 installed package modules, including namespace facades._
