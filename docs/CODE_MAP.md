...
<!-- code-map-inputs-sha256: 7fde466913be6b4e55a5c936c7f9c9936cc86609e5663e740ec490c6f934c364 -->
- **code-map inputs sha256:** `7fde466913be6b4e55a5c936c7f9c9936cc86609e5663e740ec490c6f934c364`  •  **git HEAD:** `f0d52b3`
...
- ✅ reverse module coverage clean — all 110 installed Python modules are classified (86 service implementation modules + 24 explicit support modules).
- ✅ canonical ownership clean — installed modules and all 98 declared acceptance files have no executable retired-namespace import, dynamic-import, or patch-target edge.
...
- ✅ acceptance execution lanes — default `cpu_light`; `cpu_exclusive`=7, `cpu_light`=50, `gpu_serial`=41. Every acceptance file resolves to exactly one validated lane.
...
- **`axis1_mps.py`** (3212 LOC) — service: `restricted_axis1_1d_mps` — Evaluator-side certification for restricted MCWF/MPS execution evidence.
...
- **`axis1_carrier_execution.py`** (2251 LOC) — service: `axis1_dense_jointl_record` — Executable Axis-1 carrier seam.
...
- **`axis1_mcwf_mps_execution.py`** (2813 LOC) — service: `restricted_axis1_1d_mps` — Fixed-microstep MCWF-over-MPS execution for Axis-1 carrier programs.
...
- **`mutation.py`** (3917 LOC) — Registry-driven L2 mutation runner.
...
---
_display inventory (excludes `__init__.py`): 17 packages, 110 modules, 170 public classes, 621 public functions (committed `src/` + dev `tools/`) + curated local-only clusters above. The release-contract count above separately includes all 110 installed package modules, including namespace facades._
