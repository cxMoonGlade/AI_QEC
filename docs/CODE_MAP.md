...
<!-- code-map-inputs-sha256: bc2290cd1a2c9e842606a4975feb2d74c7ce3683dff89d11145c1c44c6287292 -->
- **code-map inputs sha256:** `bc2290cd1a2c9e842606a4975feb2d74c7ce3683dff89d11145c1c44c6287292`  •  **git HEAD:** `9d04961`
...
- ✅ canonical ownership clean — installed modules and all 107 declared acceptance files have no executable retired-namespace import, dynamic-import, or patch-target edge.
...
- ✅ acceptance execution lanes — default `cpu_light`; `cpu_exclusive`=7, `cpu_light`=54, `gpu_serial`=46. Every acceptance file resolves to exactly one validated lane.
...
| **`clifford_augmented_peps_prototype`**<br>All-qubit Clifford-augmented PEPS prototype | **RESEARCH**<br>restricted_verification<br>no canonical record<br>carrier/capeps | `src/error_coupling_simulator/carrier/capeps/__init__.py`<br>`src/error_coupling_simulator/carrier/capeps/algebra.py`<br>`src/error_coupling_simulator/carrier/capeps/frame.py`<br>`src/error_coupling_simulator/carrier/capeps/residual.py`<br>`src/error_coupling_simulator/carrier/capeps/state.py` | `error_coupling_simulator.carrier.capeps:CAPEPSState`<br>`error_coupling_simulator.carrier.capeps:StimCliffordFrame` | core: numpy, stim, quimb; extras: none | All-qubit Clifford circuits, coherent signed-Pauli expansions or small-local complex128 unitaries, exact local Clifford refactors, Pauli observables, and explicit measurement outcomes; Explicit fail-closed seam for a separately installed SDIM 1.3.3 qubit tableau backend; no canonical live-SDIM acceptance environment is registered → Hybrid state satisfying \|psi> = C\|phi>; Untruncated complex128 residual-update diagnostics, Eq. (5) generator/phase ledgers, Pauli expectations, exact local-refactor diagnostics, and an ordered raw conditional-branch ledger; Opt-in bounded two-qubit bridge native-gate plans, local split ledgers, parent exact-small comparison mechanics, and one current bounded formal transient-truncation result | Engineering-mechanics prototype only. Stim is the default frame. SDIM is an unregistered, fail-closed optional seam: the adapter is qubit-only and version-pinned, while canonical acceptance covers phase translation and unavailable-backend rejection rather than live SDIM execution. The qubit GCAMPS Eq. (5) specialization uses GF(2) generator decomposition, explicit ordered-product phase recovery, and a direct signed-pullback consistency gate. Signed Pauli pullbacks preserve coherent amplitudes; arbitrary small-local unitary expansion retains all Pauli coefficients without a floating structural-zero filter. Exact one- and adjacent-two-site refactorization implements (C, phi) -> (C Q^dagger, Q phi), but the paper's 20/90-candidate search and finite-bond optimizer are not implemented. Nonlocal Quimb PEPS sums use an untruncated virtual direct sum with no finite-bond or scaling claim. PECOS is only a future isolated differential-reference candidate; no CAPEPS-PECOS comparator is implemented or accepted. The raw branch ledger is not a canonical RecordBatch, and complete multi-round QEC Record, leakage/qutrit, finite-bond, and production-faithfulness claims remain open. The dedicated 2026-07-29 closure and preregistration freeze one n=8, active-rank-3, untruncated differential between two equal-status candidate state-action lanes: plain Quimb PEPO-on-PEPS and GCAPEPS. An untimed NumPy-only exact-small anchor is frozen by design but not yet implemented. Once implemented and passing its controls, it may qualify only this one input-state action; neither candidate is ground truth and the anchor never enters an efficiency ratio. A separate 2026-07-29 preregistration froze, before implementation, one two-qubit bridge tracer comparing exact tree-PEPO action with opt-in Quimb-native one/two-site compilation under a deliberately lossy per-gate cap. The native fork implementation and focused tests are bound to commit e6cbe016f336843925e01a559db26f209fa9d37b, tree 854ff4d5ef692497f017a57250cf8f440e47110f; the parent dense anchor, guarded runner, and non-formal acceptance test are current owners. The separately executed held-out run binds parent commit 1e9517af31f83d174bcbdf656c1955f12227b605, tree 17c17eb549d5f091263e7deaa86476d90420174b, and is recorded in docs/simulator_validation/GCAPEPS_NATIVE_FORCED_TRUNCATION_RESULT_2026-07-29.md as PASS_BOUNDED_BRIDGE_TRANSIENT_TRUNCATION. Its raw temporary JSON SHA-256 is 55d428ceebb38aba91e1fbeb2e2a6d6f1b2f5da944534179ef2f583e4fa65ac7 and canonical content hash is 73ca030b410b0bf60f6fc6a1e599064ec21a5c024c8ecfd28955e8f7ad934a58. With max_bond=1 the first native CNOT discarded the positive squared tail 25/169; the cap-only complete vector had d2=d_inf=5/13 and fidelity 144/169. Exact-tree, uncapped, high-cap, and direct controls agreed with the dense anchor within 3.8e-16 over complete vectors, full-operator d_inf was 1.25e-16, the cutoff control produced the same vector with a distinct cutoff cause, and the exact final state had rank one. This supports only exact-small, two-site transient path dependence. The acceptance test remains non-formal and uses only the excluded pilot or synthetic ledgers. Local discarded weight remains forbidden as a global error, loopy-PEPS, accumulated-error, probability, measurement/reset, or Record certificate. The bridge collects no performance sample and adds no efficiency claim. Existing n=8 and d=3/5/7 artifacts remain pinned to fork commit 6fbbf74cd36686ed30a4d8865697ce46e47056c1 and are not rebound. The loopy 2x2 extension remains P1. These packets do not authorize Record-law, generic Carrier faithfulness, leakage/qutrit, generic controlled-truncation, scaling, general efficiency, or production-faithfulness claims. The separate AnySearch-backed finite-memory bond-32 closure supports only a bounded project-defined persistent-memory unitary dilation, registered dense BLP witness, and carrier-comparison design. Its repository-only implementation and scoped regression surface is green at parent commit eccae8984fa1d6e4b466af43360e67c38afde3b7, tree 3d2902edbb3afa410b4266c80d3c9f88a61389c3, with fork commit d90bb5ea210e666cbd7ecf8a8b7fa02390519baf, tree f7cd3496c48ec69f1800d41eabcaa8d53cab3b5b. Those scripts and tests are development research owners, not canonical installed-service acceptance. No formal calibration, calibration amendment, held-out report, or result note exists. The --development-direct adapter runs the adaptive calibration graph in fresh ordinary subprocesses, while the separate DEVELOPMENT_SWEEP runner executes an explicitly selected fixed grid. Both record no systemd facts, set formal_claim_eligible=false and is_heldout_evidence=false, and cannot publish a target amendment or formal held-out report; their reports and ordinary pytest are nonclaim-bearing development diagnostics. The formal CLI is explicitly fail-closed. Formal activation still requires comparator-to-classifier-to-cell_results wiring, complete amendment/cell/fixture binding, comparator semantic validation at the supervisor seam, and the preregistered noninteractive system-scope systemd preflight. Host preflight has not been rerun at this checkpoint; an authorization result would not close the implementation wiring gaps. SDIM remains qubit signed-frame corroboration only. This status adds no generic PEPS contraction, faithfulness, speedup, non-Markovian QEC/Record, qutrit/composite-d, leakage, measurement/reset, or LER claim. acceptance: tests/test_capeps_hybrid.py, tests/test_capeps_gcamps_formulas.py, tests/test_external_gcapeps_native_forced_truncation.py |
...
| **`record_contract_and_packing`**<br>Canonical detector/observable record containers and consecutive-check fold helper | **CORE**<br>record_contract<br>canonical `{det, obs}`<br>carrier/record boundary | `src/error_coupling_simulator/carrier/__init__.py`<br>`src/error_coupling_simulator/carrier/records.py`<br>`src/error_coupling_simulator/carrier/record_fold.py` | `error_coupling_simulator.carrier:RecordBatch`<br>`error_coupling_simulator.carrier:PackedShotBatch`<br>`error_coupling_simulator.carrier:pack_raw_syndrome_shots`<br>`error_coupling_simulator.carrier:s_to_det`<br>`error_coupling_simulator.carrier:det_to_s` | core: numpy; extras: none | Backend raw syndromes/observables or canonical detector/observable arrays → Validated RecordBatch/PackedShotBatch and exact consecutive-check XOR-layout fold/unfold | This service implements the current binary Record containers plus one exact consecutive-check raw-syndrome XOR-layout helper. General detector/observable parity incidence and the backend-neutral product boundary are defined by docs/SIMULATOR.md and docs/CAPABILITY_MODEL.md. The helper is not a universal detector definition, decoder, metric, capability grade, or assurance claim. acceptance: tests/test_record_batch_units.py, tests/test_carrier_record_fold.py |
...
- **`test_mutation.py`** (4534 LOC) — CPU-only self-tests for mutation-result accounting.
