...
For the still-open production notion-1 audit, the **literature target** is now frozen more narrowly:
at common post-reset cycle boundaries, keep the fixed-dimensional data register as the system, use
the same input-independent source law for every input state, average once over the complete
correlated source history, and discard ancillas and classical outcomes only after retaining their
nonselective measurement/reset backaction. This would give an unconditional family
`Lambda_r : L(H_data) -> L(H_data)`. The growing classical record tape is not part of the BLP/RHP
system; its joint law remains notion-2. Current production code does **not** yet instantiate that
family: it derives one readout/reset parameter set from the whole horizon's source trajectory, so
early-round reset and the numerical instrument change with future draws and with total `R`. It is a
fixed-horizon, path-conditioned policy rather than a causal, cross-horizon prefix-consistent map
family. See
...
operational bridge remains open; after the documented 2026-07-13 local/external/citation-chain search,
the specific literature row is `confirmed-literature-gap` **within the recorded search corpus**. This
is a search-status statement, not a global theorem that no such result can exist.
...
- **Notion-1:** corrected definition is literature-grounded. The Gaussian-surrogate null and exact
  ...
  remain open. A codespace-only input-pair search would be a restricted, cycle-sampled BLP
  witness/lower bound, not the canonical full-state-space maximum; absence of sampled revival does
  not imply CP-divisibility. A noninvertible measurement/reset map must not be handled by ordinary or
  pseudo-inversion: RHP-style discrete divisibility requires a CPTP extension on the full data
  operator space satisfying the composition equation, not only a map on `Im Lambda_s`.
- **Notion-2:** the record-memory object and full-history Markov condition are grounded. Lag-2 CMI,
  ...
  Current production does not use one source-independent fixed instrument: its readout/reset
  parameters are whole-horizon source-trajectory means. Its record law therefore mixes
  source-conditioned channel and tester/SPAM modulation unless a separate fixed-instrument arm is
  introduced.
