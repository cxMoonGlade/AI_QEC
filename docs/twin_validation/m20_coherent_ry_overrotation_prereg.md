...
|---|---|---|---|---|
| 1q RY coherent over-rotation `U_θ = exp(−iθ_Y Y)` (single-qubit `θ_Y Y` term of `H_θ = Σ_k θ_k P_k`) | Kaufmann, Rojkov & Reiter, arXiv:2307.08741 (Eq. 2: `E_{P,θ}=U_θ∘P∘U_I`, `U_θ=exp(−iH_θ)`, `H_θ=Σ_k θ_k P_k`; the single-qubit set is the 3 rotations `{θ_X X, θ_Y Y, θ_Z Z}` — `θ_Y Y` IS M20; first-order coherent in off-diagonal PTM) **+ Sheldon arXiv:1504.06597** (Eq. 1 `U=exp(−i(ε/2)r̂·σ)`, r̂=ŷ) **+ axis-specific Lazăr/Ficheux arXiv:2212.01077** (drive-amplitude mis-calibration → measured over-rotation; same pulse-area mechanism as X) — ≥2 DIRECT-physical, text-verified 2026-06-30 | observable below | `coherent_robust_pauli_2307.08741.md`, `sheldon_iterative_rb_overrotation_1504.06597.md`, `lazar_drive_nonlinearity_overrotation_2212.01077.md` | `simulator/axis1_mcwf_mps_execution.py:_hamiltonian_matrix_for_term` (family `COH_RY` → `_coherent_family_generator` → `H=(coeff/2)Y`, axis map `_ONE_Q_FAMILY_TO_AXIS["COH_RY"]="Y"`, line 1044) |
...
**Why M20 meets the ≥2-DIRECT-physical bar (shared close-reads as M6/M7 + the axis-specific Lazăr 2212.01077 amplitude ref).** The mechanism paper
