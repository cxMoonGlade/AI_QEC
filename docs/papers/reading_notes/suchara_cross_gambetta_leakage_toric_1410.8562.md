...
Our program: the **error-coupling SIMULATOR** faithfully **GENERATES the record `{det, obs}`** for the realistic non-Pauli sources (T1/T2, **leakage |1⟩→|2⟩ + seepage**, soft readout). Validity has two legs and neither is a decoder: (i) **FAITHFULNESS** — the generated record matches an INDEPENDENT oracle (exact qutrit-DM / from-scratch / closed form), with every simplification declared and bounded (`docs/FAITHFULNESS_PROTOCOL.md`); and (ii) **ANTI-TOY LEGITIMACY** — the record's correlated multi-round structure is DISTINGUISHABLE from a matched CP-divisible / Markov-`k` / best-Pauli-DEM null (Guardrail B). This paper is **directly on the leakage axis**: leakage+seepage is a genuinely **non-Markovian, NON-DEM-reducible notion-2 SOURCE** the simulator must generate faithfully (notion-3 is CLOSED as a protocol boundary — `docs/twin_validation/notion3_protocol_boundary_closure.md`; the passive record is CLASSICAL / Pauli / notion-2). It answers three needs: (a) the canonical leakage+seepage SOURCE model + how to bound the approximation vs an exact reference, (b) how to *generate* the leakage record at scale, (c) leakage/soft-info as an additional faithful OUTPUT channel of the record — decoder/LER is downstream use (`docs/METRICS.md`), never validity.
...
- **The model is, itself, the cheap approximation** of the true *coherent* qutrit leakage. The paper is explicit (Sec. 3) that it is a **stochastic proxy** ("not a worst-case stochastic model", lines 327–341) — the two-qubit mixed blocks are replaced by **Haar-random single-qubit depolarisation** of the contained partner, which **destroys leakage-induced correlations**. *For us this is the key caveat for the faithfulness leg:* if the simulator generates a Suchara-style stabilizer-compatible leakage record, the **discrepancy vs an exact qutrit-coherent reference lives precisely in those discarded correlations** (collective vs independent depolarisation of a leaked qubit's neighbours) — and that correlated multi-round structure is exactly the notion-2 signal a matched Markov/DEM null cannot forge. Per `docs/FAITHFULNESS_PROTOCOL.md`, this simplification must be declared and its error BOUNDED against an INDEPENDENT oracle: the exact qutrit density-matrix reference is `src/error_coupling_simulator/carrier/exact/qutrit_dm.py` (a small stabilizer plaquette + neighbours over a few rounds), and the scalable faithful reference is the qutrit-MPS of `leakage_tensor_network_simulation_2308.08186`. Bound it on a quantity that *is* sensitive to those correlations — the joint detector distribution / mutual information among a leaked qubit's neighbour detectors — not just a marginal. The paper itself leaves "quantifying the impact of a worst-case stochastic model to future work" (line 341): that correlation structure is exactly the notion-2 object our simulator must render faithfully and a matched null cannot reproduce.
...
- **The correlated multi-round record is the anti-toy object.** Because a leaked qubit stays leaked across rounds until it relaxes, its "L"-interval imprints correlated detector patterns on its space-time neighbours (Sec. 5) — a genuinely non-Markovian, non-DEM-reducible structure. This is precisely the notion-2 record a matched CP-divisible / Markov-`k` / best-Pauli-DEM null cannot forge (Guardrail B / anti-toy legitimacy); the leakage source is a natural generator for it because DEM reduction cannot absorb the seepage-driven cross-round memory.
...
- **The Standard-vs-HL gap quantifies how much the leakage record carries beyond a Pauli-DEM null — an anti-toy calibration.** The Standard decoder is leakage-blind (re-weights edges but ignores "L"); the HL decoder uses the herald and **roughly doubles the leakage threshold** (`β`: 3.59→1.23 Quick, 2.55→0.92 Partial-LRU; Table 2). That ≈2× is a concrete measure that the leakage record carries **correlated information a matched Pauli-DEM null cannot represent** — i.e. downstream evidence for the anti-toy legitimacy of the notion-2 leakage source, with the caveat that here it required **perfect heralding** (see Limitations). LER/threshold here is downstream product (`docs/METRICS.md`), read as an instrument on the record's non-DEM-reducibility, never as the simulator's validity.
...
- **W2 — not worst-case; correlations deliberately destroyed.** The Haar-random depolarisation in mixed two-qubit blocks (lines 327–341) makes a leaked qubit's neighbours suffer **independent** depolarisation. Real leakage (e.g. a leaked transmon coupled to neighbours) produces **collective/correlated** errors. Since the anti-toy legitimacy of the leakage source rests on exactly the cross-qubit / cross-round correlations a matched Markov/DEM null cannot forge, **this proxy under-represents the very structure the simulator must render faithfully** — treat it as a *lower-bound/baseline* generator, and BOUND the correlation gap against the exact qutrit-DM oracle (`src/error_coupling_simulator/carrier/exact/qutrit_dm.py`) / the qutrit-MPS reference (`leakage_tensor_network_simulation_2308.08186`) per `docs/FAITHFULNESS_PROTOCOL.md` (this is the "future work" the paper itself names, line 341).
