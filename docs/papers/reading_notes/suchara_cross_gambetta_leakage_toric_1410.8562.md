...
Our program: a **sim-only teacher** generates realistic-noise surface-code **syndrome** data (the non-Pauli signal — T1/T2, **leakage |1⟩→|2⟩ + seepage**, soft readout) to train a decoder, where the headroom is whatever a Pauli decoder (MWPM / TN-MLD / Google RL-prior) leaves on the table. This paper is **directly on the leakage axis** and answers parts of all three needs we listed: (a) the canonical leakage model + how to bound an approximation, (b) how to *simulate* leakage at scale, (c) the decoder's soft-/leakage-info input.

**(a) Canonical leakage model + the "approximation vs exact reference" question.**
- This is a **canonical stochastic leakage model** with two physically-motivated channels we can lift directly: **excitation** `E↑(ρ)=(1−p↑)ρ + p↑|2⟩⟨2|` (Eq. 5) and **seepage/relaxation** `E↓` with Kraus `A=|0⟩⟨0|+|1⟩⟨1|`, `A_k=(1/√2)|k⟩⟨2|` mapping a leaked qubit to the **maximally mixed contained** state (Eqs. 6–8). These are exactly the "leakage + seepage" primitives our teacher needs, in closed Kraus form. Our existing canonical T1/T2 Kraus channels compose with these on the `{|0⟩,|1⟩,|2⟩}` qutrit.
- **The model is, itself, the cheap approximation** of the true *coherent* qutrit leakage. The paper is explicit (Sec. 3) that it is a **stochastic proxy** ("not a worst-case stochastic model", lines 327–341) — the two-qubit mixed blocks are replaced by **Haar-random single-qubit depolarisation** of the contained partner, which **destroys leakage-induced correlations**. *For us this is the key caveat for the "bound the approximation error" requirement:* if we adopt a Suchara-style stabilizer-compatible leakage model as our teacher, the **error vs an exact qutrit-coherent reference lives precisely in those discarded correlations** (collective vs independent depolarisation of a leaked qubit's neighbours). The honest way to bound it is to compare against a small **exact qutrit density-matrix** simulation (our `forward/exact` backend, ≤~15 qubits, e.g. one stabilizer plaquette + neighbours over a few rounds) on a quantity that *is* sensitive to those correlations — the joint detector distribution / mutual information among a leaked qubit's neighbour detectors — not just the marginal LER. The paper itself leaves "quantifying the impact of a worst-case stochastic model to future work" (line 341): that gap is *our* contribution surface, since our non-Pauli headroom is exactly the correlation structure a Pauli decoder cannot see.
- **`α/(1+βr)` (Eq. 12) is a ready sanity-law.** Any leakage teacher we build should reproduce a monotone threshold-vs-`r` decay close to `1/(1+r)`; a Standard (leakage-blind) decoder must stay **below** the idealized `λ=3/4` depolarizing bound (Eqs. 10–12). This is a cheap *predict-before-measure* gate for our teacher's plausibility (theory-first discipline).

**(b) How to simulate leakage/non-Pauli at scale — the decisive idea for us.**
- The **"|2⟩-dephased" fixed-point lemma (Eq. 9)** is the trick that makes leakage **stabilizer-simulable at code scale**: track **one Pauli label + one leakage bit (`L`) per qubit**, no density matrix (Sec. 3, lines 343–390). This is *exactly* the simulation regime our sim-only teacher wants — it scales to surface-code `d` like ordinary Pauli-frame/Stim simulation. Concretely, the propagation rules to implement: `E↑` sets `L` (trace out the qubit); `E↓` clears `L` and injects a completely-mixed (→ uniform Pauli-frame) qubit; a sealed 2q gate on (contained, leaked) **completely depolarizes the contained input** (uniform single-qubit Pauli) and acts as identity on (leaked, leaked). A leaked **data** qubit silently drops itself from the checks it would join; a leaked **ancilla** "turns off" its check. This is the cheapest faithful leakage teacher we can stand up, and it produces **honest binary syndromes** (plus an optional per-readout "L" flag) — the right training data shape.
- **Steady-state seeding (`p_eq ≈ 4p↑/(4p↑+6p↓)`, Sec. 3)** is a detail we must copy: never-reset data qubits must start at their equilibrium leakage population, else the teacher under-states leakage in long runs. The 40%-leaked No-LRU saturation is the *positive control* that our simulator's leakage accumulation is wired correctly.
- **The cost caveat for our decoder side:** the HL conditional graph is **non-translation-invariant with extra low-weight edges**, forcing a full shortest-path matching (Boost+Blossom V, Sec. 6) instead of coordinate-distance MWPM. If we feed leakage heralds into a *matching* baseline this is the runtime hit; it is also a concrete reason our **neural/TN decoder** (which ingests the herald as an input channel rather than re-weighting a graph) could be the cheaper and more flexible consumer of leakage information.

**(c) Neural-decoder architecture + soft-info / leakage input.**
- The paper has **no neural decoder** — but it specifies *exactly what leakage information a decoder should consume and how it changes the inference*, which is the input-design spec for ours. The **3-outcome "L" herald** is a per-qubit, per-measurement classical flag, available **only at LRU/syndrome-measurement times**, that localizes a leak to an *interval* and implies **high-probability Pauli errors on the leaked qubit's space-time neighbours** with the explicit weights `p_i=i/(2n)` and edge-composition `p_new=p_0+p_i−2p_0p_i` (Sec. 5). For our decoder this maps cleanly to an **extra input channel** on the space-time syndrome volume (the binary detector tensor plus a parallel **leakage-herald tensor**, and — when readout is analog — a **soft-info/IQ tensor**), with the Sec. 5 conditional structure as the *ground-truth target a learned decoder should recover* on its own. The **missing-syndrome rule** (absent bit on an "L", zero-weight `a`-edges, defect on the earliest vertex of a contiguous "L" run) is the principled way to handle leaked readouts in any decoder, neural or matching — we should reproduce it as a masking/feature rule.
- **The Standard-vs-HL gap is precisely our "headroom above a Pauli decoder" story, validated.** The Standard decoder is leakage-blind (re-weights edges but ignores "L"); the HL decoder uses the herald and **roughly doubles the leakage threshold** (`β`: 3.59→1.23 Quick, 2.55→0.92 Partial-LRU; Table 2). That ≈2× is a clean **lower bound on the value of soft/leakage information** that a Pauli-only decoder cannot capture — i.e. a concrete instance of the gap our sim-only non-Pauli program is built to exploit, with the caveat that here it required **perfect heralding** (see Limitations).

**Cross-references in our tree.** Same leakage axis as the program memory note *decoder-gate-and-frontier* (pivot to the non-Pauli leakage/soft-readout/T1T2 teacher). Complements `marton_asboth_coherent_readout_surface_2303.04672.md` (the *coherent*+readout axis, exact FLO) — Suchara is the *leakage* axis with the opposite tradeoff (stabilizer-cheap but correlation-lossy, vs FLO exact-but-coherent-only). The exact-qutrit cross-check belongs to `forward/exact` (`docs/ARCHITECTURE.md`); the herald/soft-info input belongs to the decoder stack.

---

## 6. Limitations / what does **not** apply to us

- **W1 — optimistic perfect heralding.** The HL decoder assumes the meter reports "L" **iff** the qubit is leaked (Sec. 5). Real readout has **leakage-detection error** (false/missed "L") and, on superconducting hardware, "L" is entangled with the *analog IQ* distribution rather than a clean classifier output. The ≈2× HL benefit is therefore an **upper bound**; our soft-readout teacher must model imperfect heralding (the IQ tail of |2⟩ overlapping |1⟩), which is *more* realistic and likely the actually-interesting regime. **What carries over:** the herald-as-input idea; **what does not:** the factor-2 number as an achievable target.
- **W2 — not worst-case; correlations deliberately destroyed.** The Haar-random depolarisation in mixed two-qubit blocks (lines 327–341) makes a leaked qubit's neighbours suffer **independent** depolarisation. Real leakage (e.g. a leaked transmon coupled to neighbours) produces **collective/correlated** errors. Since our entire non-Pauli headroom thesis is *about* the correlations a Pauli decoder misses, **this model may under-represent exactly the signal we want** — adopt it as a *baseline/lower bound* leakage teacher, and quantify the correlation gap against an exact qutrit reference (this is the "future work" the paper itself names, line 341).
- **W3 — toric, translation-invariant, phenomenological-ish.** Results are on the **toric** code (periodic boundaries); planar/rotated are only *conjectured* equal (Abstract). The Standard decoding graph is translation-invariant; edge weights are single-fault counts (Sec. 4), not a full circuit DEM. Our target is the **rotated surface code on real Google layouts**, so the *numbers* (0.70 %, 0.22 %, Table 1 weights) are not directly portable — the *model* and *algorithm* are.
- **W4 — no continuous/coherent leakage dynamics, no T1/T2 inside the leak.** Leakage is a discrete per-gate Bernoulli (`p↑`) with stochastic relaxation (`p↓`); there is **no Markovian/coherent qutrit evolution**, no T1/T2 during the leaked interval, no seepage-rate physics beyond the `E_decay` Kraus. Our teacher intends *canonical T1/T2 Kraus*; we must **compose** them onto the qutrit ourselves — this paper gives the leakage/seepage half, not the relaxation-during-leakage half.
- **W5 — Standard decoder edge weights are model-specific.** Table 1 weights are derived **for this exact stochastic model and gate order**; they are not a general leakage DEM. Useful as a *worked example* of how leakage re-weights a matching graph, not as numbers to reuse.
- **W6 — decoder is MWPM, classical, offline.** No learning, no real-time constraint, no scalability study of the HL shortest-path matching (which is *more* expensive than coordinate MWPM). The decoder-design ideas transfer; the implementation does not inform our GPU/neural runtime budget.
