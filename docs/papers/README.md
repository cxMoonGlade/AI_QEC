...
| `nasr2023_counterfactual_nonidentifiability.pdf` | *Counterfactual (Non-)identifiability of Learned Structural Causal Models* (arXiv:2301.09031; see abstract for authors). | The impossibility theorem: with general multi-dim exogenous noise, counterfactuals are NOT point-identified from observational data even with known structure. The formal core of "observational ≠ interventional" — the binding reason ADR 0002 validates the loop on a controlled teacher first. Survey W1. |
...
| `qec_dem_estimation_syndrome_2504.14643.pdf` | *Estimating detector error models from syndrome data* (arXiv:2504.14643, 2025). | The DEM-from-syndrome baseline (Walsh–Hadamard). This is the **moment-matching baseline** that ADR 0003 keeps only as a negative control (it Pauli-shadows coherent structure). |
...
| `qec_differentiable_mle_noise_2602.19722.pdf` | *Differentiable Maximum Likelihood Noise Estimation for Quantum Error Correction* (arXiv:2602.19722, 2026). | Differentiable MLE of noise from syndromes — **directly parallel to `forward/exact/circuit_sim` / `recover_channel` NLL calibration**. Closest external prior art; benchmark the B-path calibration against it. |

## Bayes-TN QEC posterior models

These five papers define the Bayes-TN spine: posterior decoding over logical classes
or logical channels, plus Bayesian posterior inference over noise parameters from
syndrome histories. They are cached under `bayes_tn/`.
...
mapping it to its paper and to the ADR 0004 D-items.
...
so it serves as the ADR 0004 D4 Pauli-shadowing negative-control reference, not a
