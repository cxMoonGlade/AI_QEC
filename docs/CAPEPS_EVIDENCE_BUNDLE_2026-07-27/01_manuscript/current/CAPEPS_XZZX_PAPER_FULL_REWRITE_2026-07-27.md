...
> target。2026-07-27 theory-first 已把 exact-small qubit 的 20 个
> post-local 代表、精确 score differential 和 paired-refactor correctness 冻结为
> result-blind experiment design；但新承重文献尚未全部进入 current corpus，
> Rényi-2 score 也尚未在 `docs/METRICS.md` 绑定当前 owner 与 independent value
> test。因此 formal theory-first closure 仍为 `OPEN`，exact-small design
> maturity 是 `EXACT_SMALL_QUBIT_DESIGN_FROZEN`，preregistration gate 仍为
> `FAIL`，theory-first decision 为 `CODE_BLOCKED`；这不是新代码许可。scalable
> approximate-PEPS certificate、qutrit CAPEPS 和 primary efficiency protocol
> 继续阻塞。
...
的 held-out candidate-execution runtime，peak host/device memory 是 secondary
...
runtime on an independent holdout; peak host and device memory are
...
冻结，随后以 independent holdout 上的 candidate-execution runtime
...
2026-07-27 经用户授权的三批 AnySearch 检索与 primary-source 原文核验
给出 bounded discovery observation（不是 search-exhaustion 结论）：三批返回候选未闭合直接实现或证明
Clifford-frame + PEPS-residual 的 primary work，因此该证据行仍为
`MISSING/OPEN`，该桥接仍须标 `[ours/proposed]`；exact-small
qubit 的 post-local 20-catalogue、独立 720-candidate differential 和
exact-fidelity certificate 已闭合到 result-blind 设计层，但 source admission
与 metric registration 尚未通过，不能据此写新 optimizer code。普通
approximate CTMRG/boundary environment 的通用可扩展误差证书仍为 `OPEN`。
相邻工作 [8] 已经实质收窄 design claim，但不能替代这一明确边界。
...
basis 与 coefficient tensor network 的组合 [1]。对 Clifford gate \(G\)，只
更新 frame；对
...
则把每个 Pauli \(P_j\) 连同其 sign 拉回 coefficient state。一般 selective
measurement 的 Born probability 与 normalized conditional-state rule 由
Czajkowski--Grilo [22, Eq. (1)] 承担；它不是从 tensor-network 表示本身推出
的 accuracy certificate。GCAMPS 把 leading Clifford 与 MPS residual 组合为
...
或 finite-bond fidelity-transfer theorem。独立闭合表明，20 和 90 必须解释为
商掉 **post-action local Clifford** 的单侧陪集，而不是 input-local 或双陪集：

\[
|\operatorname{Sp}(4,\mathbb F_2)|/|\operatorname{Sp}(2,\mathbb F_2)|^2
=720/6^2=20,
\]

\[
|\operatorname{Sp}(4,\mathbb F_3)|/|\operatorname{Sp}(2,\mathbb F_3)|^2
=51840/24^2=90.
\]

Kim–Oh–Kim [12, Supplement Eqs. (S6)–(S7)] 明确使用 left cosets；矩阵乘法侧
仍随 row/column convention 改变，所以代码接口应命名
`PostLocalCosetRepresentative`。Masot-Llima 等 [14, Sec. II.B] 写出的双侧
等价不能用于 generic fixed-input score：CNOT 与
\(\mathrm{CNOT}(H\otimes I)\) 对 \(|00\rangle\) 分别给 product 与 Bell state。
真正的 two-qubit double quotient 只有四类 [15]。

本文是 all-qubit 工作，90 只属于独立的 prime-qutrit/SDIM theory lane，不是
当前实现目标。配套公式审计只验证 qubit untruncated mechanics，不把原型称为
完整 GCAMPS reproduction。
...
针对 PEPS certificate 的 disconfirmation search 还找到两个重要的条件性结果。
Schwarz–Buerschaper–Eisert [20, Theorem 1, Eqs. (1), (5)] 在 injective PEPS、
uniformly gapped parent-Hamiltonian family 和 condition-number control 下给出
局域 observable 的 quasi-polynomial approximation bound；Vanderstraeten 等
[21, Sec. IV and Eq. (B4)] 对具有相应空间对称、Hermitian transfer matrix 的
iPEPS 子类给出 algorithm-independent variational contraction objective。前者不是
任意 CAPEPS state 的 whole-state fidelity certificate，后者也不是普通 CTMRG 的
a posteriori state-error bound；故它们把 `OPEN` 精确缩小为
`OPEN_GENERIC__CONDITIONAL_CLASSES_EXIST`，而不是关闭该行。
...
证据责任必须拆开：Bonilla Ataides 与 Darmawan [5,6] 支持 XZZX stabilizer
geometry、ordered local check shell 和 consecutive-round defect；一般
selective Born update 与有序 outcome law 由 Czajkowski--Grilo
[22, Eqs. (1), (9)] 支持；每轮把 ancilla reset 到 \(|0\rangle\) 的组件由
Ghosh 等 [23, Fig. 1] 支持。上述来源都不提供本文目标的完整 absolute raw
columns、first/terminal anchors、detector/observable XOR rows 或 offsets。
这些对象必须作为 hash-frozen project input，并由 independent route 重建。
本文的 `MR_Z` 是这些组件的显式 project composition，不归属于任一来源。
...
本文的 exact-small qubit 候选集合冻结为 20 个 post-local 单侧陪集代表，
不是双陪集。令 \(G=\operatorname{Sp}(4,\mathbb F_2)\)、
\(H=\operatorname{Sp}(2,\mathbb F_2)^2\)，项目 canonical key 为

\[
k(S)=\operatorname{lexmin}_{L\in H}\operatorname{flatten}(LS\bmod2),
\]

其中矩阵侧可按 tableau convention 反向，但物理语义始终是 output-local。
验收必须独立得到 \(|G|=720\)、\(|H|=36\)、20 个互斥 orbit、每个 orbit
大小 36，并在冻结 fixtures 上证明 all-720 与 20-representative 的最优 score
和注册 tie-break 一致。

Chang 等 [11, Eq. (10), Algorithms 1–2, Theorem 1] 给出 qubit iff hash。
FOCUS 固定提交 `05b5b3a` 中正确 cross-check 是
`pyfocus/camps/file/clifford_2qubit_big.npz`，SHA-256
`466b03f9d2c59dcee5c67c9a97c348e5415b21c34b4a94b04b4fdb8aee996a8e`。
旧的 indexed `clifford-2bits-unique-entropy-big.npz` 覆盖 pre-local 方向，
在正确 post-local quotient 下只有 13 类，禁止移植其 index list。

exact-small primary objective 是冻结 physical cuts 上的 exact Rényi-2 entropy
[10, Eq. (19)]：

\[
J(Q)=\max_{e\in\mathcal E}
\left[-\log\operatorname{Tr}\rho_{A_e}(Q\phi)^2\right],
\]

并以 cut-sum 和 canonical key tie-break。它由 dense exact contraction 评估，
不声称是 scalable PEPS objective。\(q=3\) 的 90 count 已闭合，但公开权威
90-representative list 未找到；SDIM 只作为将来的 prime-qutrit execution 和
phase-ledger differential，不进入当前 qubit CAPEPS。
...
### 4.5 Exact-small fidelity budget 与 scalable boundary

对 normalized pure states 定义

\[
D(\rho,\sigma)=\tfrac12\|\rho-\sigma\|_1=\sqrt{1-F(\rho,\sigma)}.
\]

若 deterministic exact evolution 中第 \(t\) 次 finite-network compression 的
whole-state fidelity 由 exact contraction 得到为 \(F_t\)，则 project hybrid
argument 给出

\[
D_{\mathrm{final}}
\le\min\left(1,\sum_t\sqrt{1-F_t}\right).
\]

因而任意 observable error 不超过
\(2\|O\|_\infty D_{\mathrm{final}}\)，任意最终 measurement TV 也不超过该值。
对完整 enumerated classical–quantum frontier，若一次 truncation 保持 branch
weights，并且每个 branch 的 exact fidelity 为 \(F_{t,h}\)，则该次增量为

\[
\Delta_t=\sum_h w_{t,h}\sqrt{1-F_{t,h}},
\qquad
\operatorname{TV}(P_{\mathrm{raw}},\widetilde P_{\mathrm{raw}})
\le\min\left(1,\sum_t\Delta_t\right).
\]

frozen Record fold 是 classical channel，故同一上界传递到 Record law。这个
bound 不自动保证 rare normalized conditional branch，selected branches 仍须
直接做 complete-vector fidelity。上述式子是 `[ours]` 推导，不是 PEPS 论文的
原结论。

Evenbly [16, Eq. (12)] 支持 exact full-environment whole-network fidelity 作为
finite PEPS truncation objective，但 alternating solve 没有 global-convergence
定理。若 contraction 只给 \(\widehat a,\widehat b,\widehat c\)，则只有另有
certified absolute errors \(\eta_a,\eta_b,\eta_c\) 时才能写
...
本轮没有找到普通 CTMRG/boundary approximation 产生通用 \(\eta\) 的方法；
PEPS exact contraction 又具有 worst-case complexity 障碍 [4]。因此 exact-small
certificate 可预注册，scalable approximate-environment correctness 继续
`CODE_BLOCKED`。

### 4.6 当前 untruncated-mechanics 证据及其边界
...
### 4.7 Finite-bond state-and-Record acceptance gates
...
### 5.1 Neutral instrument、to-be-frozen workload 与 independent lowering

target requires a to-be-hash-frozen、backend-neutral XZZX instrument specification，
至少包含：
...
目标还必须在预注册前选择 coherent \(R_Y\) 的坐标定义。由于
\(HYH=-Y\) 且 \(H R_Y(\theta)H=R_Y(-\theta)\)，若 local-H 只作用于一部分
data qubits，则物理坐标中 uniform \(R_Y(+\theta)\) 在 transformed XZZX
fixture 中会变成 sign-staggered rotation。可选的另一目标是在 transformed
fixture 中直接定义 uniform \(R_Y(+\theta)\)，但它是不同的 project fixture，
不得声称等价于 physical-uniform noise。qubit subset、sign pattern、gate
order、measurement basis 与 fold coordinates 必须一起冻结并独立重建。
...
execution task、three non-overlapping splits（pilot selection、certification
holdout、timing holdout）、dtype、same hardware、configuration
...
\(d_{\mathrm{code}}=5\) does not enter matched registered correctness-gate efficiency because no
...
对每个 primary route
\(r\in\{\mathrm{full\ PEPS},\mathrm{CAPEPS}\}\)，冻结三个不重叠集合：
pilot/selection set、independent certification holdout 与 independent timing
holdout；同时冻结 grid \(\mathcal C_r\)、equal tuning budget 和 deterministic selection rule。查看 pilot 前，为两条 primary routes
...
1. 保留 theory-first prerequisite gate：先把新承重 source-only notes
   正式收入 current corpus，并为 exact Rényi-2 score 注册当前 owner 与
   independent value test；在此之前 exact-small 20-catalogue 也不得进入新
   `src/**`，scalable approximate environment 与 qutrit CAPEPS 继续 fail closed。
...
Theory-first 本轮结果是设计证据，不是 optimizer 数值结果：exact-small qubit
20-catalogue 已有经纠错的 result-blind design freeze；formal closure 为 `OPEN`，
preregistration gate 为 `FAIL`，theory-first decision 为 `CODE_BLOCKED`；
承重 source admission 与
Rényi-2 metric registry 关闭之前不能执行。scalable approximate-PEPS optimizer、
prime-qutrit CAPEPS 和 efficiency target 仍为 `CODE_BLOCKED/PENDING`。下面既有
mechanics 与 baseline 数字不能回填该预注册。
...
3. **全数据 \(R_Y\) 可能扩散 residual。** 它不是 sparse-magic regime，可能
   快速耗尽任何 residual bond advantage。与此同时，local-H 变换满足
   \(H R_Y(\theta)H=R_Y(-\theta)\)；physical-uniform 与
   transformed-uniform 两种 fixture 不能混写，当前 target 尚未在两者中
   冻结一个。
...
5. **optimizer 只有 exact-small 定义，且前置门仍开着。** 20 个 post-local
   representatives、exact physical-cut Rényi-2 objective、tie-break 与 all-720
   differential 已写入 result-blind preregistration，但承重 source admission 与
   metric owner/value-test registration 尚未完成，也尚未执行。有环 PEPS 的 scalable environment、
   gauge-aware compression objective、cadence、stopping 和 certified contraction
   error 仍未闭合；exact-small PASS 不能转移成 scalable claim。
...
9. **primary efficiency experiment 尚未 preregister。** 新的 exact-small
   catalogue preregistration 不包含 runtime winner。accuracy bands、pilot/holdout
...
中心科学问题仍然开放，但理论门禁已拆开。下一步先独立复核并执行
exact-small 20-versus-720 catalogue preregistration，再完成 genuine-2D 与
enumerated multi-step tracers。scalable approximate PEPS environment 在取得
certified contraction-error construction 前保持阻塞。随后才可为
\(d_{\mathrm{code}}=3\) 冻结 population `PASS` 或 sampled `PASS*` reference
设计、pilot selection 和 independent timing holdout。\(d_{\mathrm{code}}=5\)
仍只承担 provisional reachability，twirled tableau 只承担 different-channel
trade-off；prime-qutrit SDIM 是独立后续路线。
...
[9] X. Qian, J. Huang, and M. Qin, “Augmenting Density Matrix
Renormalization Group with Clifford Circuits,” *Physical Review Letters*
**133**, 190402 (2024). [arXiv:2405.09217](https://arxiv.org/abs/2405.09217)

[10] Z. Liu and B. K. Clark, “Classical Simulability of Clifford+T Circuits
with Clifford-Augmented Matrix Product States,” *Physical Review Research*
**8**, 023116 (2026).
[arXiv:2412.17209](https://arxiv.org/abs/2412.17209)

[11] L. Chang et al., “Clifford Disentanglers for Entanglement Reduction in
Molecular Electronic Structure Simulations,” arXiv:2606.12056v1 (2026).
[arXiv:2606.12056](https://arxiv.org/abs/2606.12056)

[12] M. Kim, C. Oh, and D. Kim, “Disentangling Haldane Phase by Generalized
Clifford Circuits,” arXiv:2607.03939v1 (2026).
[arXiv:2607.03939](https://arxiv.org/abs/2607.03939)

[13] E. Hostens, J. Dehaene, and B. De Moor, “Stabilizer States and Clifford
Operations for Systems of Arbitrary Dimensions, and Modular Arithmetic,”
*Physical Review A* **71**, 042315 (2005).
[quant-ph/0408190](https://arxiv.org/abs/quant-ph/0408190)

[14] S. Masot-Llima et al., “Limits of Clifford Disentangling in Tensor
Network States,” *Physical Review B* **114**, 024311 (2026).
[arXiv:2602.15942](https://arxiv.org/abs/2602.15942)

[15] A. D. Córcoles et al., “Process Verification of Two-Qubit Quantum Gates
by Randomized Benchmarking,” *Physical Review A* **87**, 030301 (2013).
[arXiv:1210.7011](https://arxiv.org/abs/1210.7011)

[16] G. Evenbly, “Gauge fixing, canonical forms, and optimal truncations in
tensor networks with closed loops,” *Physical Review B* **98**, 085155
(2018). [arXiv:1801.05390](https://arxiv.org/abs/1801.05390)

[17] P. Czarnik, J. Dziarmaga, and P. Corboz, “Time evolution of an infinite
projected entangled pair state: An efficient algorithm,”
arXiv:1811.05497 (2018). [arXiv:1811.05497](https://arxiv.org/abs/1811.05497)

[18] A. H. Werner et al., “A Positive Tensor Network Approach for Simulating
Open Quantum Many-Body Systems,” *Physical Review Letters* **116**, 237201
(2016). [arXiv:1412.5746](https://arxiv.org/abs/1412.5746)

[19] S. M. Li et al., “A Complete and Natural Rule Set for Multi-Qutrit
Clifford Circuits,” *EPTCS* **426**, 23–78 (2025).
[arXiv:2508.14670](https://arxiv.org/abs/2508.14670)

[20] M. Schwarz, O. Buerschaper, and J. Eisert, “Approximating Local
Observables on Projected Entangled Pair States,” *Physical Review A* **95**,
060102(R) (2017). [arXiv:1606.06301](https://arxiv.org/abs/1606.06301)

[21] L. Vanderstraeten et al., “Variational Methods for Contracting Projected
Entangled-Pair States,” *Physical Review B* **105**, 195140 (2022).
[arXiv:2110.12726](https://arxiv.org/abs/2110.12726)

[22] J. Czajkowski and A. B. Grilo, “On-State Commutativity of Measurements
and Joint Distributions of Their Outcomes,” arXiv:2101.08313v2 (2021).
[arXiv:2101.08313](https://arxiv.org/abs/2101.08313)

[23] J. Ghosh, A. G. Fowler, J. M. Martinis, and M. R. Geller,
“Understanding the Effects of Leakage in Superconducting Quantum-Error-Detection
Circuits,” *Physical Review A* **88**, 062329 (2013). [arXiv:1306.0925](https://arxiv.org/abs/1306.0925)
...
- CAPEPS XZZX Record-efficiency literature closure and code gate:
...
- Harper adjacent hybrid-QEC source audit:
...
- Disentangler theory-first closure and prerequisite-blocked code gate:
  [`CAPEPS_DISENTANGLER_THEORY_FIRST_CLOSURE_2026-07-27.md`](CAPEPS_DISENTANGLER_THEORY_FIRST_CLOSURE_2026-07-27.md)
- Result-blind exact-small catalogue preregistration:
...
The current theory-first disposition is `OPEN` with
`EXACT_SMALL_QUBIT_DESIGN_FROZEN` design maturity and a `FAIL` preregistration
gate; its implementation decision is `CODE_BLOCKED`: the source-cache
hash is repaired and the exact-small 20-catalogue claim has a corrected,
result-blind preregistration, but new source admission and the Rényi-2 metric
owner/value-test gate remain open. Genuine-2D mechanics, canonical Record,
scalable finite-bond/environment certification, numerical efficiency gates,
and statistical target preregistration also remain open, so all those target
executions remain stopped.
