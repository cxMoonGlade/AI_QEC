...
| M3 | **DONE 2026-06-10** (`metric_results.md` M3 RESULTS; **P3 gate PASS both bases** — held-out ΔNLL(naive − twin) +56.2 X / +44.3 Z nats/shot/window at one-sided 99%; drift-isolated fallback corroborates at +56.7 / +46.3) | Findings: the pij independent-edges arm OVERSHOOTS (P5 reversed in both bases — the P10 budget-deficit consequence: deficit positive at 366–1116σ); basis-independent round-repeat bunching ≈5.3 at 4.4σ (X excess ⇒ non-dephasing-symmetric finding; per-qubit f̂ point values not fiber-resolved in X — abstained per P9, the resolved functionals are flip-rate/bunching); inter-sample drift ≈44 nats/shot (M5 feed). Execution: 160 fits on the RTX 5090 via static-Kraus-input CUDA graphs (84 fits/22 min, single context; bit-exact vs amended eager at three pin levels — ledger adjudications 6–7 + amendment). |
