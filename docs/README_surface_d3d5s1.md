...
Stored in `b8` format, see The b8 Format. In the parsing function, the `bits_per_shot` argument should be the total number of measurements in the circuit. Each shot's data is byte aligned by padding up to a multiple of 8 bits. Bits are packed into bytes in little endian order.
---
```
...
├── sweep_bits.b8
...
Stored in `b8` format, see The b8 Format. In the parsing function, the `bits_per_shot` argument should be the number of observables in the circuit, which in this dataset is always 1.
---
```
...
├── metadata.json
...
correlated_matching_decoder_with_si1000_prior	Correlated matching decoder based on the sparse blossom matching engine [1] with a variant of the two-step re-weighting strategy [2].	SI1000 prior [3], inspired by the typical hierarchy of physical error rates in the superconducting qubits.
correlated_matching_decoder_with_rl_optimized_prior	Same as correlated matching above.	Prior optimized with reinforcement learning [4] for the correlated matching decoder. Optimized for each patch / basis in isolation using the 13-cycle calibration data.
harmony_decoder_with_si1000_prior	Harmony decoder [5] ensembling 101 correlated matching decoders.	Same as SI1000 prior above.
...
Related dataset acquired on Google's Sycamore processor, released in Ref. [4].
Another related dataset acquired on Google's Sycamore processor, released in Ref. [6].
