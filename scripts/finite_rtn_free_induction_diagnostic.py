#!/usr/bin/env python3
"""Exact finite-RTN free-induction research diagnostic.

The post-result execution contract is
``docs/simulator_validation/finite_rtn_free_induction_diagnostic_contract_2026-07-15.md``.
The script tests two explicitly declared single-qubit diagnostic lifts of the current
``OneOverFDriftSource`` endpoint process. It does not test the production parameter fan-out,
scheduled QEC channel, syndrome record, or downstream estimator.
"""
...
SCHEMA = "error_coupling_simulator.source.finite_rtn_free_induction_diagnostic.v1"
...
PROTOCOL_STATUS = "POST_RESULT_RECONSTRUCTION_NOT_PREREGISTERED"
...
def summarize_diagnostics(
    *, implementation_passed: bool, continuous_verdict: str, held_verdict: str
) -> str:
    """Summarize scientific outcomes without turning an implementation pass into a positive result."""

    if not implementation_passed:
        return "IMPLEMENTATION_GATE_FAILED"
    if continuous_verdict == held_verdict == "CONFIRMED_DIAGNOSTIC_ONLY":
        return "BOTH_DIAGNOSTICS_POSITIVE"
    if continuous_verdict == held_verdict == "NULL_WITHIN_HORIZON":
        return "BOTH_DIAGNOSTICS_NULL_WITHIN_HORIZON"
    return "MIXED_DIAGNOSTIC_RESULT"


def execution_provenance() -> dict[str, Any]:
    """Bind the run to its actual import, clean tracked inputs, locks, and runtime."""

    root = Path(__file__).resolve().parents[1]
    production_source = root / "src/error_coupling_simulator/source/process.py"
    imported_source = inspect.getsourcefile(OneOverFDriftSource)
    if imported_source is None or Path(imported_source).resolve() != production_source.resolve():
        raise RuntimeError(
            "OneOverFDriftSource import origin does not match the bound production source: "
            f"imported={imported_source!r}, bound={production_source}"
        )
    paths = {
        "script": Path(__file__).resolve(),
        "diagnostic_contract": root / DIAGNOSTIC_CONTRACT,
        "production_source": production_source,
        "environment_lock": root / "core-environment-cu130.lock",
    ...
    return {
        ...
        "import_origin": {
            "module": OneOverFDriftSource.__module__,
            "source_file": production_source.relative_to(root).as_posix(),
        },
        "runtime": {
            ...
            "packages": {
                ...
                for name in ("numpy", "scipy", "mpmath", "error-coupling-simulator")
            },
        },
    }
...
def build_report() -> dict[str, Any]:
    """Execute every registered diagnostic gate and return a JSON-safe report."""
    ...
    # The continuous-lift verdict is analytic: the display grid cannot create
    ...
    diagnostic_summary = summarize_diagnostics(
        ...
        continuous_verdict=ctmc_verdict,
    ...
    report: dict[str, Any] = {
        ...
        "diagnostic_contract": DIAGNOSTIC_CONTRACT.as_posix(),
        "claim_boundary": {
            "tested": [
        ...
        "implementation_verdict": "PASS" if implementation_passed else "FAIL",
...
def report_content_hash(report: dict[str, Any]) -> str:
    """Hash canonical report content while excluding the self-referential hash field."""
...
def write_report_atomic(path: Path, report: dict[str, Any]) -> str:
    """Atomically publish one report and return the exact-byte SHA-256."""
...
def main() -> None:
    ...
    report = build_report()
    byte_hash = write_report_atomic(args.output, report)
    print(
        "FINITE_RTN_FREE_INDUCTION_DIAGNOSTIC",
        "implementation=",
