#!/usr/bin/env python3
"""Run the 170-call mixed-shape plan with ``torch.linalg.svd`` instead of pinv.

Single-variable discrimination arm: identical manifest, GA/GB snapshots,
side/shape/order trace, per-call pre/post synchronization cadence, and durable
event boundaries as the pinv reproducer; only the solver call changes (the
plan's rtol is carried in the record but not applied).  A hard reset here
implicates the solver-class kernel burst rather than pinv's specific
composition.  This is a runtime stress/reproduction test.  It is not pytest,
simulator acceptance, scientific evidence, or a hardware-health certificate.
"""
...
RESULT_SCHEMA = "error_coupling_simulator.runtime.svd_170_diagnostic.v1"
...
DEFAULT_OUTPUT_PARENT = (
    REPO_ROOT
    / "outputs/simulator_validation/runtime_reset_watch/svd_170_standalone_runs"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--preflight-only",
        action="store_true",
        help="validate inputs and runtime without executing svd",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    common = {
        ...
        "single_variable_change": (
            "pinv replaced by svd on the identical plan and sync cadence; "
            "plan rtol recorded but not applied"
        ),
    ...
    if args.preflight_only:
        result = {
            **common,
            ...
            "claim_boundary": "no svd call was executed",
        }
    ...
    try:
        for call in executed_calls:
            ...
            events.emit(
                "svd_returned_before_explicit_sync",
                call=asdict(call),
                calls_completed=calls_completed,
                tensor=tensor_metadata(svd_s),
            )
            if target.type == "cuda":
                torch.cuda.synchronize(target)
            health = tensor_health(svd_s)
            health["u_finite"] = bool(torch.isfinite(svd_u).all().item())
            health["vh_finite"] = bool(torch.isfinite(svd_vh).all().item())
            if not (health["finite"] and health["u_finite"] and health["vh_finite"]):
                raise ValueError(f"svd result for call {call.call_index} is non-finite")
            calls_completed += 1
            ...
            print(
                ...
                f"elapsed={elapsed:.3f}s smax={health['abs_max']:.9e} finite=true",
                flush=True,
            )
    ...
    result = {
        **common,
        ...
        "claim_boundary": (
            "constructed svd runtime diagnostic only; not exact historical tensor replay, "
            "simulator acceptance, scientific evidence, or hardware-health certification"
        ),
    }
    atomic_json(run_root / "result.json", result)
    events.emit(
        "svd_sequence_terminal",
        status=result_status,
        calls_completed=calls_completed,
        sequence_elapsed_s=sequence_elapsed,
