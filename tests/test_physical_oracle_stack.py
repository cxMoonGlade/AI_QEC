...
from scope_static.physical_oracle import stack as stack_mod


def test_physical_oracle_paths_and_artifact_loaders(tmp_path: Path) -> None:
    paths = physical_oracle_stack_paths(tmp_path)
...
def test_physical_oracle_stack_runs_all_stages_when_teacher_self_passes(tmp_path: Path, monkeypatch) -> None:
    calls: list[str] = []
    _patch_stack_adapters(monkeypatch, calls=calls, phys2_ari=1.0, phys2_nmi=1.0)

    result = run_physical_oracle_stack({"shots": 8}, output_dir=tmp_path, bootstrap_replicates=2, random_baseline_trials=3)
    ...
    assert (tmp_path / "physical_oracle_stack.json").exists()
    assert (tmp_path / "physical_oracle_stack.md").exists()


def test_physical_oracle_stack_skips_learner_when_teacher_self_is_probe_limited(tmp_path: Path, monkeypatch) -> None:
    calls: list[str] = []
    _patch_stack_adapters(monkeypatch, calls=calls, phys2_ari=0.2, phys2_nmi=0.3)

    result = run_physical_oracle_stack({"shots": 8}, output_dir=tmp_path)
...
def test_physical_oracle_stack_always_mode_runs_learner_for_diagnostics(tmp_path: Path, monkeypatch) -> None:
    calls: list[str] = []
    _patch_stack_adapters(monkeypatch, calls=calls, phys2_ari=0.2, phys2_nmi=0.3)

    result = run_physical_oracle_stack({"shots": 8}, output_dir=tmp_path, run_local_inverse="always")
...
def _patch_stack_adapters(monkeypatch, *, calls: list[str], phys2_ari: float, phys2_nmi: float) -> None:
    ...
    def fake_phys3(*, teacher_dir, separability_dir, output_dir, config):
        ...
        return {
            ...
            "acceptance_label": "physical_oracle_strong_recovery",
            "num_clusters": 2,
    ...
    monkeypatch.setattr(stack_mod, "_generate_physical_teacher_dataset", fake_teacher)
    monkeypatch.setattr(stack_mod, "_run_oracle_separability_audit", fake_phys2)
    monkeypatch.setattr(stack_mod, "_run_physical_local_inverse_discovery", fake_phys3)
