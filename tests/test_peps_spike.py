...
file was authored.  Ambiguous signature details are concentrated in the ADAPTER
block below (ONE place to fix at integration — contract §1 discipline).

Coverage (the test-builder's contract rows, §4 + §6.3):
  * SW0  — ``build_codestate_peps`` dense |psi><psi| == ``QutritDM.init_logical``
           rho (BOTH m, chunked, 1e-12); structural |2>-mass EXACTLY 0.0 on the
           tensors ((a) zero-tolerance); <S_g> = +1 for all 8 stabs and
           <Z_L> = (-1)^m READ THROUGH THE PRODUCTION CAPS PATH at the d3 floor
           (<= 1e-12, §4 grades; the dense bridge is the cross-check, never the
           source — the rung-1 A3 lesson); per-edge raw dims == 2^(chain
           multiplicity) with the multiplicity map computed IN THIS TEST from
           ``PepoLayout.plaquette_path`` BEFORE the state is built (RT1).
  * SW1  — mirrored (PepsState, QutritDM) pair through the SAME random token
           program (H/X/LEAK; leak-pump non-vacuity preconditions): per-op 1-site
           gate dense equality <= 1e-12; forced-Kraus LEAK (engine ``K_k psi``
           UNNORMALIZED vs referee ``apply_channel([K_k])``); forced-outcome
           ``sqrt(E_s)`` branch, BOTH outcomes, UNNORMALIZED, <= 1e-12.
  ...
  * SW3  — single-wire TT ranks: <= (3,5,3) w=4 / (3,) w=2 ((a) bound, asserted
           ...
           identity; above-bound impossible).
  * SW5  — harness-level draw-order/convention units on CPU value stubs: the
           sampling maps must match mps_forward's (SF11, binding):
           ``sbit = 0 iff u < p0`` (STRICT <), terminal ``bit = 1 iff u < p1``,
           leak tie-break "first k with u*tot <= cumsum_k" (NON-strict <=,
           fallback K-1 — the batched_mps_backend_prereg TIE-BREAK REGISTRY).
           The SW5 byte-identity gate itself (vs mps_forward at exact_chi) is a
           gate script, not pytest.
  * SW6  — the KILLER set (K-catalog discipline; every sabotage image DEMONSTRATED
           beyond KILLER_FLOOR = 1e-6, each with its non-vacuity precondition):
           b <-> (1-b) coherent double-swap (ENGINE-invoking, K-5); wrong-outcome-
           sign branch (K-2); corrupt-stab letter swap, path-preserving (K-2/K-8);
           ...
           the machinery is defensive/unreachable in-domain — this killer FORCES
           the branch and asserts the flag entry).
  * §6.3 — eps_l known-answer units: the product-state SMOKE (class (a) identity
           with NO discriminating power — at bond dim 1 the transfer matrix is a
           scalar and eps_l == 0 for ANY assembly, RT2); the DISCRIMINATING
           hand-written 2x2 bond-2 loop with a NONZERO eps_l, matched at 1e-10 to
           an INDEPENDENT dense eigendecomposition written from scratch IN THIS
           FILE (no shared BP code).
...
# --------------------------------------------------------------------------- #
WG_L1_TARGET = 5.0e-3   # p1c physical cell (rung-1 test constants, kept for comparability)
G_SEEP = 0.09
G_HEAT = 0.0
B_BIAS = 0.9            # the registered evidence point for the TT rank asserts (SW3/WP3)
ARM = "A"               # arm A only (D2); arm C is FENCED OUT (SW-S2)
TOL_EXACT = 1e-12       # unit-gate dense-equality bar (§4)
TOL_TRACE = 1e-10       # the SW2 branch-trace / caps-read bar (§4)
KILLER_FLOOR = 1e-6     # a sabotage image must differ by MORE than this (>> TOL_EXACT)
D3_CAPS_FLOOR = 1e-12   # §4 (a)-identity caps-path read floor at d3 (near-exact contraction)

_CUDA = torch.cuda.is_available()
requires_cuda = pytest.mark.skipif(
    not _CUDA, reason="CUDA unavailable — the PEPS engine is GPU-only (SW-S8)")
...
# integration (contract §1: the module is built in parallel; §2/§3 pin the    #
# SEMANTICS and most NAMES, not every argument spelling).                     #
# =========================================================================== #
...
def _build_codestate(sched, m: int):
    """§2 ``build_codestate_peps(sched, m)`` — unit-norm single-wire codestate."""
...
def _dense_psi(state) -> torch.Tensor:
    """§3 ``dense_psi(state)`` — the d3 bridge; engine position 0 = MSB factor."""
...
def _apply_1site(state, U: torch.Tensor, pos: int) -> None:
    """§2 ``apply_gate_1site(U, pos)`` — 1-site unitary, bond-inert."""
    fn = _resolve("apply_gate_1site")
    if fn is not None:
        _try_calls([lambda: fn(state, U, int(pos))], "apply_gate_1site")
        return
    meth = getattr(state, "apply_gate_1site", None)
    if callable(meth):
        meth(U, int(pos))
        return
    pytest.fail("carrier/peps exposes no apply_gate_1site (§2 pins it) — fix this adapter")


def _apply_kraus_forced(state, K: torch.Tensor, pos: int) -> None:
    """SW1 forced-Kraus LEAK: apply ``K_k`` as a plain 1-site linear op, psi left
    UNNORMALIZED (the engine side of ``apply_channel([K_k])``).  A dedicated forced
    seam is tried first; ``apply_gate_1site`` (the module's ``(state, U, pos)``
    ...
    for nm in names:
        ...
        cands = []
        if fn is not None:
            cands.append(lambda f=fn: f(state, K, int(pos)))
        meth = getattr(state, nm, None)
        if callable(meth):
            cands.append(lambda mm=meth: mm(K, int(pos)))
        for c in cands:
            try:
                c()
                return
                ...
                errs.append(f"{nm}: {e}")
    pytest.fail(f"no forced-Kraus 1-site apply found (tried {names}) — fix this "
...
def _stab_tt(paulis: dict, outcome: int, b: float, arm: str, layout):
    """§2 ``stab_tt_singlewire(paulis, outcome, b, arm)`` — the UNSQUARED sqrt(e)
    diagonal TT (SF3)."""
    fn = _resolve_any(("stab_tt_singlewire",), "the single-wire stab TT builder (§2)")
    return _try_calls(
...
def _apply_stab_branch(state, tt) -> None:
    """§2 ``apply_stab_branch(state, tt)`` — unnormalized sqrt(E_s) psi, no
    truncation inside."""
    ...
    if callable(meth):
        meth(tt)
        return
    pytest.fail("carrier/peps exposes no apply_stab_branch (§2 pins it) — fix this adapter")
...
def _caps_read(state, caps: dict) -> complex:
    """The PRODUCTION double-layer caps read <psi| (x)_q M_q |psi> (§2/SF9 —
...
def _obs_prob_seam(state, logical: dict, isx: dict, b: float, m: int) -> float:
    """§3 terminal row: the distribution-level exact-P(obs) seam (the
    ``terminal_readout_obs_prob`` port).  Convention pin (SF11 value contract:
    probabilities carry the /<psi|psi> normalization, mps_forward's
    ``p1 = <F1>/<psi|psi>``): the seam returns P(obs = 1) of the NORMALIZED state."""
    fn = _resolve_any(("terminal_readout_obs_prob", "terminal_obs_prob",
                       "obs_prob_exact"), "the exact-P(obs) seam (§3)")
...
def _bond_profile_norm(state) -> dict:
    """§2 ``bond_profile(state)`` normalized to {(p, q): dim} with p < q; falls
    back to reading ``B{a}_{b}`` ind sizes off the tn (naming pinned by D3)."""
    raw = None
...
def _entry_from(result, state) -> dict:
    ...
    dict appended to ``state.ledger`` (the pepo ntu_truncate convention, SF10)."""
...
def _truncate_lossless(state, bond) -> dict:
    """SW2 structural LOSSLESS policy: no cut below the exact local rank (the
    sigma > 1e-12*sigma_1 count — the pinned zero-drop convention).  Dedicated
    spellings first; registered fallback: the D_cap arm at the bond's CURRENT dim
    (kept rank = min(dim, exact local rank) = the exact local rank — structurally
    the same policy by the §2 semantics)."""
    errs = []
    for nm, calls in (
...
def _truncate_dynamic_forced(state, bond, eps_spike: float, w_max: int):
    """The dynamic-eps policy with the precut window FORCED to bind (SW6's
    window-binding killer; §6.1 pins the machinery as defensive/unreachable
    in-domain, so the killer must force it through the window knob).  Returns
    ``(entry_or_None, raised_or_None)`` — an orderly-stop raise is a legitimate
    §6.1 outcome and is handed back for the flag assert."""
    errs = []
        ...
        for eps_kw in ("eps_spike", "eps", "epsilon"):
            for w_kw in ("w_max", "W_max", "window_max", "window_cap"):
                for extra in ({}, {"policy": "dynamic"}):
                    ...
                    except Exception as e:  # noqa: BLE001 — §6.1 orderly-stop route
                        return None, e
    pytest.fail(f"no dynamic-eps truncation spelling matched (window knob needed "
                f"by the SW6 killer) — fix this adapter; {errs}")


def _has_window_binding(ledger: list) -> bool:
    """True iff the ledger carries a §6.1 ``window_binding`` flag entry."""
    for e in ledger:
...
def _map_sbit():
    """SW5 pure decision map ``(u, p0) -> sbit`` (sbit = 0 iff u < p0, STRICT <)."""
    return _resolve_any(
        ("sbit_from_uniform", "sample_sbit", "sbit_map", "stab_bit_from_uniform",
         "sbit_from_u"),
        "the stabilizer-outcome decision map (SW5; expose the pure map "
        "(u, p0) -> sbit so the convention is CPU-testable)")


def _map_terminal_bit():
    """SW5 pure decision map ``(u, p1) -> bit`` (bit = 1 iff u < p1)."""
    return _resolve_any(
        ("terminal_bit_from_uniform", "readout_bit_from_uniform",
         "terminal_bit_map", "bit_from_uniform", "terminal_bit_from_u"),
        "the terminal-bit decision map (SW5; expose the pure map (u, p1) -> bit)")


def _map_leak_branch():
    """SW5 pure decision map ``(u, [p_k]) -> k`` (first k with u*tot <= cumsum_k,
    NON-strict <=, fallback K-1 — the TIE-BREAK REGISTRY, verbatim)."""
    return _resolve_any(
        ("leak_branch_from_uniform", "kraus_branch_from_uniform", "leak_select",
         "select_kraus_branch", "leak_branch_from_u", "leak_tie_break"),
        "the leak tie-break decision map (SW5; expose the pure map (u, pk) -> k)")
...
class _DuckGrid:
    ...
    instrument may consult) for the SYNTHETIC §6.3 known-answer networks.
...
def _wrap_synthetic_state(tn, d: int):
    """Wrap a hand-built quimb TN (pinned D3 naming) for the eps_l instrument:
    try the PepsState constructor shapes, else hand back the bare TN with a duck
    layout attached (the instrument's §2 row takes 'the norm network of the
    current psi' — a bare TN is an admissible carrier of that)."""
    lay = _DuckGrid(d)
...
def _eps_l_call(obj):
    """§2 ``eps_l(state)`` — per-loop eps_l table (§6.3 conventions)."""
    fn = _resolve_any(("eps_l", "eps_l_table", "eps_l_loops", "loop_eps_l"),
                      "the eps_l instrument (§6.3)")
    errs = []
...
def _eps_loop_stats(result) -> list[tuple[float, float]]:
    """Normalize the eps_l table to ``[(eps_max, eps_mean)]`` per loop (§6.3
...
def _qutrit_mat(name: str, device="cuda") -> torch.Tensor:
    """Test-local single-qutrit operator table (LITERAL formulas — the contract's
    ...
    referee with the SAME explicit matrix, so no shared gate-table code path can
    hide a convention drift (K-8)."""
...
def _pair_maxabs(state, eng) -> float:
    """UNNORMALIZED dense equality of the mirrored pair (SW1 convention)."""
    return _psi_rho_maxabs(_dense_psi(state), eng.rho)


def _pair_maxabs_normalized(state, eng) -> float:
    """Normalized-image comparison (SW6 killer instrumentation: sabotage teeth
    ...
    require_precondition(n2 > 1e-12 and tr > 1e-12,
                         ...
                         f"normalized comparison undefined (K-7)")
...
def _site_trits(idx: torch.Tensor, site: int, n: int) -> torch.Tensor:
    ...
    referee module-docstring convention, recomputed here independently)."""
    return (idx // (3 ** (n - 1 - int(site)))) % 3


def _site_rdm(rho: torch.Tensor, site: int, n: int = 9) -> torch.Tensor:
    """1-site reduced density matrix off the dense referee rho (view-based
...
def _obs_prob_dense(rho: torch.Tensor, logical: dict, b: float, m: int, *,
    ...
    """The pinned obs-law composition, hand-built on the DENSE referee rho and
    ...
    LOGICAL support ONLY, XOR m.  ``swap_b=True`` builds the FORBIDDEN coherent
    double-swap image (the SW6 K-5 sabotage reference).  Returns P(obs = 1)."""
    n = round(math.log(rho.shape[0]) / math.log(3))
...
def _resolve_logical_z(sched) -> dict:
    """The oracle-wiring logical operator (SF1 / build_codestate step 3, mirrored
    here for the caps identities): ``sched.logical`` only when ``logical_kind``
    is 'Z'; else the referee's ``{0: 'Z'}`` fallback."""
...
def _chain_multiplicity_map(sched):
    """SW0's REGISTERED per-edge chain-multiplicity map, computed from the actual
    ``PepoLayout.plaquette_path`` outputs BEFORE the state is built (RT1): chains
    = the 8 stabilizers in ``sched.stabilizers`` order THEN the logical (SF1);
...
def wc(sched):
    ...
    # prereg-C4 on the referee-side Kraus (also the §2 leak_sample precondition)
...
def _pick_jump_k(rdm: torch.Tensor, kraus: list) -> tuple[int, int]:
    """(dominant, jump) Kraus indices off the referee-side 1-site RDM: dominant =
    ...
    drives both engine and referee, so the choice is instrumentation, not a
    certified map)."""
    pks, w2s = [], []
    for K in kraus:
    ...
    require_precondition(bool(live), "no non-degenerate leak branch to force (K-7)")
...
def _prep_pair(wc, seed: int, *, n_ops: int = 5, pump_sites: tuple[int, ...] = (),
    ...
    """A mirrored (PepsState, QutritDM) pair through the SAME random token program
    (SW1 shape: H/X 1-site gates + LEAK).  Every LEAK token is a FORCED-Kraus
    application, UNNORMALIZED on both sides (engine ``K_k psi``; referee
    ...
    for i, (tok, site) in enumerate(prog):
        ...
        if per_op_bar is not None:
            ...
            assert worst <= per_op_bar, (
                f"SW1 per-op equality: op {i} ({tok}@{site}) max-abs {worst:.3e} > "
                f"{per_op_bar:.1e}")
    tr = float(torch.diagonal(eng.rho).real.sum().item())
    require_precondition(tr > 1e-8, f"prep trace collapsed to {tr:.3e} — the "
                                    f"unnormalized comparisons would be vacuous (K-7)")
    return state, eng
...
def _grown_engine_state(wc, seed: int):
    ...
    matrix column — no referee needed), then every w4 stab branch (outcome 0)
    ...
    require_precondition(scores[k_jump] > 0.0,
                         "no |2>-inflow branch in the leak set (K-7)")
    for site in (0, 4):
...
# =========================================================================== #
# SW0 — codestate: dense bridge, structural |2>-mass, caps-path identities,    #
# per-edge 2^multiplicity bond law                                             #
# =========================================================================== #
@requires_cuda
class TestSW0Codestate:
    @pytest.mark.parametrize("m", [0, 1])
    def test_dense_psi_outer_matches_oracle_rho(self, wc, m):
        # §4 SW0: dense |psi><psi| vs QutritDM.init_logical(m).rho, chunked
        # max-abs <= 1e-12, BOTH m (never a full 5.77-GiB comparison temp).
        state = _build_codestate(wc["sched"], m)
        psi = _dense_psi(state)
        assert psi.shape == (_N9,), psi.shape
        assert psi.dtype == torch.complex128, psi.dtype       # SW-S8: c128 everywhere
        n2 = float((psi.conj() * psi).real.sum().item())
        ...
        assert worst <= TOL_EXACT, f"SW0 m={m}: max-abs {worst:.3e} > 1e-12"
        del state, eng, psi
        _free()

    def test_structural_two_mass_exactly_zero_on_tensors(self, wc):
        # §4 grades: |2>-mass is checked STRUCTURALLY on the tensors — the k=2
        # slice of EVERY site tensor is exactly 0.0 ((a) zero-tolerance; survives
        # any contraction).  Also the D3 representation pins: k{pos} dim 3,
        # torch-cuda-complex128 (SW-S8).
        state = _build_codestate(wc["sched"], 0)
        tn = state.tn
        for pos in range(9):
            tag = f"Q{pos}"
            ...
            assert t.data.is_cuda, "site tensor not on CUDA (SW-S8)"
            kname = f"k{pos}"
    ...
    def test_caps_path_stabilizer_and_logical_identities(self, wc, m):
        # §4 SW0: <S_g> = +1 for ALL stabs and <Z_L> = (-1)^m READ THROUGH THE
        ...
        # never the source (the rung-1 A3 lesson).  Reads are formed as the ratio
    ...
    def test_per_edge_dims_equal_two_to_chain_multiplicity(self, wc):
        # §4 SW0 + SF1 (RT1): per-edge raw dim == 2^(chain multiplicity through
        # that edge).  The multiplicity map is computed from the ACTUAL
        # plaquette_path outputs (8 stabs in schedule order + the oracle-wiring
        # logical) and REGISTERED here BEFORE the state is built.
        lay, mult = _chain_multiplicity_map(wc["sched"])          # registered first
        state = _build_codestate(wc["sched"], 0)
        prof = _bond_profile_norm(state)
        for edge in lay.grid_edges():
            want = 2 ** mult.get(edge, 0)
            got = int(prof.get(edge, 1))     # structurally-empty edge == dim 1 (§2)
            assert got == want, (
                f"edge {edge}: raw bond dim {got} != 2^multiplicity = {want} "
                f"(registered multiplicity map: {mult})")
        del state
        _free()


# =========================================================================== #
# SW1 — mirrored per-op pair: 1-site gates, forced-Kraus LEAK, forced-outcome  #
# sqrt(E_s), all UNNORMALIZED at 1e-12                                         #
# =========================================================================== #
@requires_cuda
class TestSW1MirroredPerOp:
    def test_random_token_program_per_op_equality(self, wc):
        # §4 SW1: the SAME random token program (H/X/LEAK) on both carriers,
    ...
    def test_forced_kraus_leak_dominant_and_jump_branches(self, wc):
        # §4 SW1: forced-Kraus LEAK — engine K_k psi UNNORMALIZED vs referee
        # apply_channel([K_k]) (SF7: single-element list, no CPTP assert), for the
        ...
        for label, k in (("dominant", k_dom), ("jump", k_jump)):
            ...
            require_precondition(tr_branch > 1e-8,
                                 ...
                                 f"degenerate — comparison vacuous (K-7)")
    ...
    def test_forced_outcome_sqrt_es_branch_unnormalized(self, wc, which, outcome):
        # §4 SW1: forced-outcome sqrt(E_s) branch, BOTH outcomes, UNNORMALIZED,
        # engine TT vs referee project_stabilizer (SF5 pattern), <= 1e-12.
        ...
        require_precondition(float(eng.rho.abs().max().item()) > 1e-6,
                             ...
                             "unnormalized comparison (K-4/K-7)")
...
# =========================================================================== #
# SW2 — the structural LOSSLESS policy unit (the chain probe is a script)      #
# =========================================================================== #
@requires_cuda
class TestSW2LosslessPolicy:
    def test_lossless_cut_preserves_state_and_exact_local_rank(self, wc):
        """SW2 (pytest slice): the LOSSLESS policy is STRUCTURAL — no cut may
        reduce rank below the exact local rank (sigma > 1e-12*sigma_1 count, the
        pinned zero-drop convention; RT1: a weight bar is fp-junk-trippable).
        Certified on the contracted image: the dense psi is UNCHANGED at 1e-12
        by a lossless cut on a grown bond; ledger fields (pepo convention, SF10)
...
# =========================================================================== #
# SW3 — single-wire TT ranks: (a) bound always; == at b = 0.9 is WP3           #
# =========================================================================== #
@requires_cuda
class TestSW3SingleWireTTRanks:
    def test_tt_ranks_at_registered_evidence_point(self, wc):
        # §4 SW3 + SF3: bounds (3,5,3) w=4 / (3,) w=2 are class (a) — rank ABOVE
        # is impossible; measured EQUALITY at (arm A, b=0.9) is the WP3
        # expectation (below-bound contradicts the parent's executed equality via
        # the Kronecker-square identity => porting bug, never a mechanism note).
        w4, w2 = _pick_stabs(wc["sched"])
        lay = _pepo_layout(wc["sched"])
        ...
        assert r4 == (3, 5, 3), (
            f"WP3 miss: w=4 ranks {r4} != (3,5,3) at b={B_BIAS} arm {ARM} — "
            f"below-bound = porting bug (Kronecker-square identity, SF3/RT1)")
        tt2 = _stab_tt(w2, 0, B_BIAS, ARM, lay)
        r2 = _tt_ranks(tt2)
        assert len(r2) == 1 and r2[0] <= 3, (
            f"(a)-bound violation: w=2 single-wire TT ranks {r2} exceed (3,)")
        assert r2 == (3,), (
            f"WP3 miss: w=2 ranks {r2} != (3,) at b={B_BIAS} arm {ARM}")


# =========================================================================== #
# SW5 — draw-order/convention value contracts on CPU stubs (SF11, binding)     #
# =========================================================================== #
class TestSW5SamplingMapContracts:
    """The three sampling maps are PURE decision functions; their conventions are
    pinned VERBATIM to mps_forward's (SF11/D7).  Reference maps below are written
    from the CONTRACT text, not imported — a shared code path could not fail."""

    def test_stab_map_sbit_zero_iff_u_strictly_below_p0(self):
        smap = _map_sbit()
        # (u, p0, want): boundary u == p0 -> 1 (STRICT <; the K-3 class)
    ...
    def test_leak_tie_break_nonstrict_cumsum_fallback_last(self):
        ...
        pk = [0.25, 0.25, 0.5]
        # boundary u*tot == cumsum_k selects k (NON-strict <= — the registry pin)
        ...
        for u, want in cases:
            ...
            assert got == want, (
                ...
                f"TIE-BREAK REGISTRY pins first k with u*tot <= cumsum_k "
                f"(non-strict), fallback K-1")
        # a zero-weight FIRST branch at u = 0 is selected by the non-strict <=
        # (0 <= cumsum_0 = 0) — the sharpest registry pin
        assert int(lmap(0.0, [0.0, 1.0])) == 0
        # u = 1.0 lands on the last branch (via non-strict compare or fallback K-1
        # — the registry makes the two routes indistinguishable by value)
        assert int(lmap(1.0, [0.3, 0.3])) == 1


# =========================================================================== #
# SW6 — the KILLER set (each sabotage image DEMONSTRATED > KILLER_FLOOR, each   #
# with its non-vacuity precondition)                                           #
# =========================================================================== #
class TestSW6KillersMaps:
    """CPU-stub killers (pure decision maps)."""
    ...
    def test_k_det_s_fold_triple_agreement_and_orientation_sabotage(self):
        ...
        # sabotage image: the wrong-orientation (suffix) fold
        ...
        assert img > KILLER_FLOOR, (
            f"orientation-fold sabotage image {img} <= {KILLER_FLOOR} — the fold "
            f"gate has no teeth")


@requires_cuda
class TestSW6KillersEngine:
    """Engine-invoking killers (real-patch fixture, CUDA)."""

    def test_k_b_double_swap_obs_seam_engine_invoking(self, wc):
        """K-5 killer, ENGINE-invoking: the exact-P(obs) seam must MATCH the
        ...
        require_precondition(abs(p_swap - p_ok) > KILLER_FLOOR,
                             f"b-swap does not move the dense obs probability "
                             f"(|{p_swap:.6e} - {p_ok:.6e}| <= {KILLER_FLOOR}) — "
        ...
        assert -1e-9 <= p_eng <= 1.0 + 1e-9, (
            f"obs seam returned {p_eng!r} outside [0, 1] — the SF11 normalized-"
    ...
    def test_k_wrong_outcome_sign_breaks_branch_equality(self, wc):
        """K-2 killer: a flipped E_s outcome sign in the single-wire TT (engine
        outcome 1 vs referee outcome 0) must diverge beyond KILLER_FLOOR on the
        normalized branch images.  Non-vacuity: BOTH outcome branches carry
        weight on the prep (else the sabotage branch is degenerate)."""
        ...
        p0w = eng.project_stabilizer(w4, 0, B_BIAS, ARM)   # referee TRUE branch stays
        ...
        require_precondition(p0w > 1e-4 * parent_tr and p1w > 1e-4 * parent_tr,
                             ...
                             f"p1w={p1w:.3e}) — the sign killer would be vacuous")
        ...
        assert worst > KILLER_FLOOR, (
            f"outcome-sign sabotage did NOT trip (normalized diff {worst:.3e}) — "
            f"the branch gate has no teeth")
        del state, eng
        _free()

    def test_k_corrupt_stab_letter_swap_path_preserving(self, wc):
        """K-2/K-8 killer: flip ONE support site's X/Z letter (wrong support
        content, SAME support => same plaquette path, so the TT still builds) —
        the engine branch must diverge from the TRUE referee update beyond
        KILLER_FLOOR (normalized images)."""
        w4, _ = _pick_stabs(wc["sched"])
        ...
        worst = _pair_maxabs_normalized(state, eng)
        assert worst > KILLER_FLOOR, (
            f"CorruptStab sabotage did NOT trip (normalized diff {worst:.3e}) — "
            f"the stab-branch gate has no teeth")
        del state, eng
        _free()

    def test_k_conj_layer_sabotage_unconjugated_bra(self, wc):
        """K-6/K-8 killer: the double-layer caps read built with the UNconjugated
        ...
        caps read must match the correct image and reject the sabotage image."""
        ...
        require_precondition(im_rel > 1e-6,
                             ...
                             f"{im_rel:.3e}) — the conj sabotage would be vacuous")
        site = 4
        ...
        # sabotage image = the M-term with the UNCONJUGATED bra (psi^T M psi) over
        ...
        require_precondition(abs(q_sab - q_ok) > KILLER_FLOOR,
                             f"unconjugated-bra image does not separate "
                             f"(|{q_sab!r} - {q_ok!r}| <= {KILLER_FLOOR})")
        n_read = _caps_read(state, {})
        ...
        assert abs(q_eng - q_sab) > KILLER_FLOOR, (
            f"caps-path read {q_eng!r} matches the UNCONJUGATED-bra sabotage image "
            f"{q_sab!r} — the conj layer is broken (K-6)")
        del state, eng, psi
        _free()

    def test_k_ledger_image_discard_zero_exactly_when_off(self, wc):
        """The discard-ledger image killer: a truncation that DROPS REAL WEIGHT
        ...
        the strict > 0.0 assert is demonstrated to trip that sabotage.

        Single-wire d3 exact ranks stay LOW (WP3), so a real-weight PRECUT
    ...
    def test_k_window_binding_forced_flag(self, wc):
        """§6.1 forced-flag killer: the window/flag machinery is DEFENSIVE
        (unreachable while code matches spec below W_max) — this killer FORCES a
        ...
        §6.1 outcomes are legitimate: a clean return after the one retry, or the
        orderly-stop raise — the flag entry must exist either way.  (eps here is
        a forcing knob for the killer, not a float-floor claim.)"""
        ...
        assert flagged, (
            ...
            f"{_ledger(state)[-3:]}) — the §6.1 flag machinery is dead and the "
            f"zero-entries rule for a clean WP1 verdict is unenforceable")
        del state
        _free()


# =========================================================================== #
# §6.3 — eps_l known-answer units (WP2 preconditions)                          #
# =========================================================================== #
def _tn_2x2(site_arrays: dict, device="cuda"):
    """A hand-built 2x2 single-loop TN in the PINNED D3 naming (tags Q{pos},
...
# hand-written loop-correlated bond-2 site tensors (§6.3 (ii) — the
# DISCRIMINATING anchor; k = 2 slices zero, k in {0, 1} literal values chosen
...
class TestEpsLKnownAnswers:
    def test_eps_l_product_state_smoke_zero(self):
        """SMOKE ONLY (§6.3 (i), RT2): class (a) identity with NO discriminating
    ...
    def test_eps_l_2x2_bond2_loop_matches_independent_dense(self):
        """§6.3 (ii) — the DISCRIMINATING known-answer anchor: hand-written
        ...
        nonzero mismatch that the zero branch cannot mask (RT2)."""
