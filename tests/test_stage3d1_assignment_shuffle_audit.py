...
from scope_static.mechanism_discovery.assignment_shuffle_audit import run_stage3d1_assignment_shuffle_audit


def test_stage3d1_assignment_shuffle_collapses_generator_toward_null(tmp_path: Path) -> None:
    ...
    output = tmp_path / "S3D1"

    result = run_stage3d1_assignment_shuffle_audit(
        stage3a_dir=s3a,
        stage3a5_dir=s3a5,
        stage3b1_dir=s3b1,
        stage3c_dir=s3c,
    ...
    assert result["decision"] == "stage3d1_assignment_shuffle_audit_passed"
    assert result["claim_boundary"]["keeps_visible_features_fixed"] is True
    assert result["claim_boundary"]["shuffles_discovered_assignment_rows"] is True
    ...
    collapse = result["assignment_shuffle_metrics"]["primary_collapse_report"]
    assert collapse["shuffled_mean_minus_original_degradation"] > 0.0
    assert collapse["collapse_fraction_toward_global_null"] >= 0.1
    assert collapse["original_assignment"] < collapse["global_null"]
    assert collapse["shuffled_assignment_mean"] > collapse["original_assignment"]

    summary = result["shuffled_assignment_metrics_summary"]
    assert summary["all_visible_features_fixed"] is True
    assert summary["all_assignment_column_distributions_preserved"] is True
    assert summary["all_row_alignments_changed"] is True

    for run in result["shuffle_runs"]:
        assert run["visible_features_fixed"] is True
        assert run["assignment_column_distribution_preserved"] is True
        assert run["row_stochastic"] is True
...
def test_stage3d1_config_wrapper_runs_from_yaml(tmp_path: Path) -> None:
    ...
    config = tmp_path / "stage3d1.yaml"
    config.write_text(
        "\n".join(
            [
                "stage3d1_assignment_shuffle_audit:",
                f"  stage3a_dir: {s3a}",
                f"  stage3a5_dir: {s3a5}",
                f"  stage3b1_dir: {s3b1}",
                "  stage3c_dir:",
    ...
    result = run_stage3d1_assignment_shuffle_audit_from_config(config_path=config)

    assert result["decision"] == "stage3d1_assignment_shuffle_audit_passed"
    assert (output / "assignment_shuffle_metrics.json").exists()


def test_stage3d1_rejects_zero_shuffle_count(tmp_path: Path) -> None:
    _teacher, s3a, s3a5, s3b1, _s3c = _prepare_artifacts(
        tmp_path,
        [
            ("M0", "M0"),
    ...
    result = run_stage3d1_assignment_shuffle_audit(
        ...
        stage3b1_dir=s3b1,
        stage3c_dir=None,
        output_dir=tmp_path / "S3D1",
        shuffle_count=0,
    )

    assert result["decision"] == "stage3d1_assignment_shuffle_audit_failed"
