"""GPU certification of connected-cluster joint exponentiation.

Overlapping, non-identical Hamiltonian supports must be joined into one
connected-cluster generator and exponentiated once. Splitting them into
separate exact-support exponentials is a Lie-Trotter approximation and drops
the within-substep commutator contribution.

The independent references are
``carrier.joint_lindbladian.assemble_substep_channel`` (sum-all-H → one ``expm(L dt)``,
and a from-scratch mixed-radix lift in this file. A separate-support
factorization is implemented only as a corruption falsifier and is not used by
the carrier or either reference.
...
This scientific acceptance file requires CUDA.

Run: conda run -n ecs python -m pytest -q tests/test_axis1_connected_cluster_channels.py
"""
...
if not cuda_ok:
    pytest.fail(
        "connected-cluster channel certification requires CUDA",
        pytrace=False,
    )
...
def _separate_support_group_gates(substep, *, dt_ns, local_dims, device):
    """Corruption falsifier that exponentiates each exact support separately."""
    groups, order = {}, []
...
def _term(family, support, coefficient):
    return {
        ...
        "substep_id": "connected_cluster_cert",
    }
...
# Tests
# --------------------------------------------------------------------------- #
def test_overlap_corruption_fails_and_cluster_matches_reference():
    """A Trotterized support split fails while the connected cluster matches."""
    t1 = _term("LEAK_EXCHANGE_12", (0,), 0.07)
    t2 = _term("LEAK_MOBILITY_12_21", (0, 1), 0.05)
    substep = {"substep_id": "connected_cluster_cert", "terms": [t1, t2]}

    joint = _joint_window_unitary([t1, t2], dt_ns=DT, num_qubits=2, local_dims=DIMS)
    split_U = _window_unitary_from_gates(
        _separate_support_group_gates(substep, dt_ns=DT, local_dims=DIMS, device=DEVICE),
        num_qubits=2, local_dims=DIMS,
    )
    clustered = _new_gates(substep)
    clustered_U = _window_unitary_from_gates(clustered, num_qubits=2, local_dims=DIMS)
    ...
    assert len(clustered) == 1 and clustered[0]["support"] == (0, 1)
    assert _one_minus_fe(joint, split_U) > CORRUPTION_MIN
    assert _one_minus_fe(joint, clustered_U) <= TOL_PASS


def test_connected_cluster_matches_dense_channel_reference():
    """The cluster gate matches the independently assembled dense channel."""
    t1 = _term("LEAK_EXCHANGE_12", (0,), 0.07)
    t2 = _term("LEAK_MOBILITY_12_21", (0, 1), 0.05)
    substep = {"substep_id": "connected_cluster_cert", "terms": [t1, t2]}
    ...
    assert _one_minus_fe(U_oracle, clustered_U) <= TOL_PASS
...
def test_disjoint_clusters_stay_factored():
    ...
    substep = {"substep_id": "connected_cluster_cert", "terms": [t_a, t_b]}
    clustered = _new_gates(substep)
    clustered_U = _window_unitary_from_gates(clustered, num_qubits=3, local_dims=DIMS)
...
def test_chain_closes_transitively():
    """H(0,1)+H(1,2) share qubit 1 and form one transitive cluster."""
    t_ab = _term("LEAK_MOBILITY_12_21", (0, 1), 0.05)
    t_bc = _term("LEAK_MOBILITY_12_21", (1, 2), 0.045)
    substep = {"substep_id": "connected_cluster_cert", "terms": [t_ab, t_bc]}
    ...
    assert len(clustered) == 1 and clustered[0]["support"] == (0, 1, 2)
    assert _one_minus_fe(joint, split_U) > CORRUPTION_MIN
    assert _one_minus_fe(joint, clustered_U) <= TOL_PASS


def test_same_support_terms_share_one_exponential():
