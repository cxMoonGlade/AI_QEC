"""Per-CZ leakage-transport channel derived from a two-transmon Hamiltonian.
...
BASIS / OUTPUT CONVENTION
-----------------------------------------------------------------------------
The engine ``QutritDM.apply_channel_2site`` (src/error_coupling_simulator/carrier/exact/qutrit_dm.py) wants a
...
This module is the package-owned channel constructor. The scripted ground-truth
check is independent validation, not a required data file or runtime service.
"""
...
try:
    ...
    raise ModuleNotFoundError(
        "CZ leakage-channel derivation requires the core QuTiP dependency; "
        "reinstall error-coupling-simulator"
    ) from exc
...
def build_cz_channel(
    ...
    apply):
      - 'identity_sink' (default, RECOMMENDED): the leaked-out population is RETURNED to the
        ...
        a level we do not track); for a |2>/|3>-truncated carrier the bounded choice is to KEEP it
