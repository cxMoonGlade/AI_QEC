...
vectorized index named ``k{pos}`` with the row-major vec convention

    ``k = 3 * t_ket + t_bra``.

This module owns:

  * :func:`pepo_trace` — exact ``Tr(rho)``.
  ...
  * :func:`stab_expectation` — the two-term ``E_s`` decomposition
    ``Tr(E_s rho) = 1/2 Tr(rho) + 1/2 (-1)^s Tr(rho * (x)_q M_q)``.
  * :func:`terminal_readout_obs` — the declared observable law (logical support only,
    ``isx`` sites H-conjugated, ``F1 = |1><1| + b|2><2|``, ``F0 = |0><0| + (1-b)|2><2|``,
    parity XOR ``m``).
  * :func:`terminal_readout_obs_prob` — exact ``P(obs = 1)`` composed through the
    same per-site effect construction the sampling path uses.
  * :func:`s_to_det` / :func:`det_to_s` — re-exports of the carrier-neutral
    :mod:`carrier.record_fold` detector conventions
    (``det(0,j) = s(0,j)``; ``det(r,j) = s(r,j) XOR s(r-1,j)``) and inversion
    ``s(r,j) = XOR_{r'<=r} det(r',j)``.
  * :func:`negativity_witness` / :class:`NegativityStats` — explicit positivity diagnostics;
    positivity is never assumed.

Column convention: a boundary-MPS
...
GPU + complex128: every cap/effect tensor is created ``torch.complex128`` on
...
from ..record_fold import det_to_s, s_to_det  # carrier-neutral re-export

CDTYPE = torch.complex128

#: Default absolute scale for the PEPO Born-negativity diagnostic. Claim-bearing
#: runs must pass their explicitly derived threshold instead of treating this
#: implementation default as physical evidence.
NEGATIVITY_BAR_FLOOR = 4.8e-4

#: One-site variational fit constants. The fitting uses the
...
def _qutrit_hadamard(device) -> torch.Tensor:
    ...
    (leaked levels are H-inert). Built locally by formula so the implementation
    remains independent of the dense reference."""
...
def _cap_vector(cap, device) -> torch.Tensor:
    ...
    Derivation for the ``k = 3*t_ket + t_bra`` row-major convention: the site tensor's
    ...
    Accepted inputs:
      ...
      * length-81 tensor ``S`` (reshaped ``(9, 9)``, row-major, ``k_out``/``k_in`` both in
        the fused convention) -> full fused-leg superoperator: cap ``= t^T S``
...
def _fit_compress_rows(tn: "qtn.TensorNetwork", d: int, max_bond: int) -> "qtn.TensorNetwork":
    ...
    # Backend-matched initial guess: quimb's default random guess is
...
def pepo_trace(state) -> complex:
    ...
    whole single-layer network with an optimized path. The normalized state satisfies
    ``pepo_trace(state) == 1`` within the logged trace-shift ledger after every round.
    The boundary-MPS route (:func:`norm_cache` + :func:`expect_site_caps`) is the
    truncated read; this function is the exact anchor at d3 scale.
    """
    layout = state.layout
    tensors = []
...
def norm_cache(state, R_n: int) -> NormCache:
    """Build the reverse-pass boundary-MPS cache at boundary dimension ``R_n``.

    Columns are swept ``d-1 -> 1``; each absorption
...
def expect_site_caps(state, caps: dict, cache: NormCache | None = None,
    ...
    """``Tr(rho * (x)_q Pi_q)`` with general single-site caps.

    ``caps`` maps engine position -> a ``(3, 3)`` site operator, a length-9 fused
    diagonal, or a length-81 fused superoperator (see :func:`_cap_vector` for the
    ...
    Contraction routes:
      * ``cache is None and R_n is None`` -> exact contraction for d3 dense-reference
        comparisons at the ``1e-10`` tolerance;
...
# --------------------------------------------------------------------------- #
# Stabilizer expectation through the two-term E_s decomposition               #
# --------------------------------------------------------------------------- #
def _arm_leaked_weight(b: float, arm: str) -> float:
    """The per-site leaked parity weight ``d_q(leaked)`` per measurement arm.

    ``QutritDM._povm_diag_weight`` uses A/C -> ``1-2b``; B1 -> ``+1``;
    B2 -> ``-1``; unknown raises."""
    a = str(arm).upper()
    if a in ("A", "C"):
        bval = float(b)
...
def _stab_site_ops(paulis: dict, b: float, arm: str, device) -> dict:
    ...
    (the rotate-measure-rotate identity
...
def stab_expectation(state, paulis: dict, outcome: int, b: float, arm: str,
    ...
    """Raw ``Tr(E_s rho)`` via the two-term decomposition:

        ``Tr(E_s rho) = 1/2 Tr(rho) + 1/2 (-1)^s Tr(rho * (x)_q M_q)``

    with ``M_q = diag(1, -1, d2)`` on Z sites and ``H diag(1, -1, d2) H`` on X sites
    (``d2`` the arm's leaked weight). Unnormalized: divide by ``Tr(rho)`` for the
    ...
    never enter, so ``P(s)`` is identical to arm A). Dynamics applies arm-C's
    per-site fused-leg dephase mask, whose entry is one iff
    ``(t>=2) == (t'>=2)``. The mask acts on the support before the TT in
    dephase-then-``E_s`` order, so it belongs to the selective-update path, not
    this expectation read.

    Both terms use the same contraction route: exact when ``cache`` and ``R_n`` are
    both ``None``, boundary-MPS otherwise.
    """
...
# --------------------------------------------------------------------------- #
# Born-negativity diagnostics                                                   #
# --------------------------------------------------------------------------- #
class NegativityStats:
    """Run-level O(1) accumulator for the mean-negative-mass rule.

    A per-call ledger rescan would make the witness O(N^2) over a run's Born draws.
    The run driver owns one
...
def negativity_witness(q_raw: float, tr: float, ledger: list, negativity_bar: float,
                       stats: "NegativityStats | None" = None,
                       log_only: bool = False) -> None:
    """Evaluate two explicit Born-negativity stop rules:

      (i)  any SINGLE raw Born weight ``q_raw < -(10 * negativity_bar)``  =>  STOP;
      (ii) the MEAN over ALL Born draws of the run (per-draw, not per-shot-aggregated)
           of ``max(0, -q_raw) / Tr(rho)``  ``> negativity_bar``          =>  STOP.
    ...
    evaluated on the run-level :class:`NegativityStats` accumulator ``stats``, updated
    here in O(1) per draw. When ``stats`` is ``None``, rule (i) is still
    ...
    ``log_only=True`` records a would-trip without aborting. This is suitable only for
    explicitly labeled diagnostic runs where contraction-estimator error and physical
    negativity have not been separated. Normal runs retain fail-closed behavior.
    """
    bar = float(negativity_bar)
    if not bar > 0.0:
        raise ValueError(
            f"negativity_bar must be positive (got {negativity_bar})")
    q = float(q_raw)
    t = float(tr)
    ...
    if q < -(10.0 * bar):
        msg = (f"NEGATIVITY_STOP: single raw Born weight q_raw={q:.6e} "
               f"< -(10 x negativity bar) = "
               f"{-(10.0 * bar):.6e} (rule i)")
        if log_only:
            ledger.append({
                "kind": "negativity_would_trip",
                "rule": "i",
                "q_raw": q,
                "tr": t,
                "neg_mass": neg_mass,
    ...
    if stats is not None:
        ...
        if mean_neg > bar:
            msg = (f"NEGATIVITY_STOP: mean per-draw negative Born mass "
                   f"{mean_neg:.6e} > negativity bar {bar:.6e} over "
                   f"{stats.count} draws (rule ii)")
            if log_only:
                ledger.append({
                    "kind": "negativity_would_trip",
                    "rule": "ii",
...
# --------------------------------------------------------------------------- #
# Terminal-readout observable law                                              #
# --------------------------------------------------------------------------- #
def _terminal_site_effects(logical: dict, isx: dict, b: float, device) -> dict:
    """Per-site terminal-readout effect pairs over the sorted logical support.

      * ``F1 = |1><1| + b |2><2|``, ``F0 = |0><0| + (1-b) |2><2|`` (``F0 + F1 = I``;
        ``F0 - F1 = diag(1, -1, 1-2b)``);
      * each X-flagged site (``isx[q]`` truthy) H-CONJUGATED: ``E_c = H F_c H``.

    Both :func:`terminal_readout_obs` and :func:`terminal_readout_obs_prob` use
    this single effect-construction path, so the bias-swap corruption test
    exercises the implementation's own effects.
    """
    bval = float(b)
    if not 0.0 <= bval <= 1.0:
        raise ValueError(f"readout bias b must be in [0, 1] (got {bval})")
...
def terminal_readout_obs(state, logical: dict, isx: dict, b: float, m: int, rng, *,
                         negativity_bar: float | None = None,
                         stats: NegativityStats | None = None,
                         log_only: bool = False) -> int:
    """Sample the declared terminal-readout observable law:
    ...
    the independent dense reference implements the same biased-``b`` composition).
    Each per-site draw runs the negativity witness. ``negativity_bar`` is resolved once
    and passed to every witness call with the run-level accumulator and ``log_only`` flag.
    """
    device = torch.device(state.device)
    bar = (float(negativity_bar) if negativity_bar is not None
           else NEGATIVITY_BAR_FLOOR)
    if stats is None:
        # Mark that the run-level rule is inactive without an accumulator.
        state.ledger.append({
            "kind": "negativity_mean_rule_inactive",
    ...
    for q, (e0, e1) in effects.items():
        ...
        if den <= NUMERICAL_ZERO:
            raise RuntimeError(
                f"NEGATIVITY_STOP: nonpositive conditional trace {den:.6e} in terminal readout "
                f"at site {q}")
...
def _validated_conditional_probability(weight: float, total: float, *, where: str) -> float:
    ...
    if not np.isfinite(probability) or probability < 0.0 or probability > 1.0:
        raise RuntimeError(
            f"NEGATIVITY_STOP: illegal conditional probability {probability!r} in {where} "
...
def terminal_readout_obs_prob(state, logical: dict, isx: dict, b: float, m: int) -> float:
    """Exact ``P(obs = 1)`` for the declared terminal-readout observable law.

    The current corruption test calls this engine-side function and distinguishes
    the declared composition from a bias-swapped dense construction.
    ...
    exactly the discrepancy the diagnostic must expose, never mask). The state is NOT
    ...
    if tr <= NUMERICAL_ZERO:
        raise RuntimeError(
            f"NEGATIVITY_STOP: nonpositive trace {tr:.6e} in terminal_readout_obs_prob")
