...
"""Minimal Axis-1 G2 frontend bridge.

This is the first bridge from compiler-produced `SubstepSchedule` metadata to
the existing `forward.joint_lindbladian` G2 diagnostics. It is intentionally
...
G2_ZETA_RAD_PER_NS = 2.0 * math.pi * 0.37e-3
G2_GAMMA_PHI_PER_NS = 1.0 / 30000.0
G2_GAMMA_1_PER_NS = 1.0 / 30000.0
G2_DR_ZZ_BAND = (6.0e-4, 3.0e-3)
G2_FIXED_OMEGA_BAND = (4.0e-4, 4.0e-3)
G2_NONZERO_COMMUTATOR_MIN = 1.0e-6
G2_NONZERO_SUPEROP_DISTANCE_MIN = 1.0e-3
G2_DT_GRID = (20.0, 22.5, 25.0, 27.5, 30.0)
G2_DT_SMALL = (4.0, 6.0, 8.0, 10.0)
G2_DT_CONTROL = (20.0, 25.0, 30.0)
G2_FIXED_OMEGA_RAD_PER_NS = math.pi / 25.0
G2_SLOPE_TOL = 0.35
G2_ALLOWED_SOURCE_KINDS = frozenset({"circuit_ir", "code_spec_compiler", "stim_circuit"})
G2_FORBIDDEN_OUTPUT_SUFFIXES = frozenset({".stim", ".dem", ".b8", ".npz", ".npy"})
G2_FORBIDDEN_OUTPUT_NAMES = frozenset(
    {
...
class Axis1G2Row:
    """One G2 evidence row tied to a compiler-generated substep."""
    ...
    def to_manifest(self) -> dict:
        ...
        if self.mechanism_pair == ("DR", "ZZ"):
            predicted_band = list(G2_DR_ZZ_BAND)
            in_band = bool(G2_DR_ZZ_BAND[0] <= self.one_minus_F_e <= G2_DR_ZZ_BAND[1])
        if exact_zero:
...
@dataclass(frozen=True)
class Axis1G2EvidenceResult:
    """Files and manifest returned by `write_axis1_g2_evidence`."""
...
@dataclass(frozen=True)
class Axis1G2FreezeResult:
    """Freeze manifest returned by `freeze_axis1_g2_evidence`."""
...
def axis1_g2_frontend_gate(
    ...
    *,
    ...
    selection_plan = build_axis1_g2_selection_plan(schedule)
...
def axis1_g2_gate_manifest(
    schedule: SubstepSchedule,
    *,
    device: str = "cuda",
) -> dict:
    """Return a serializable manifest for the minimal G2 frontend gate."""

    rows = axis1_g2_frontend_gate(schedule, device=device)
    selection_plan = build_axis1_g2_selection_plan(schedule)
    details = _axis1_g2_anti_toy_details(device=device)
    passed = all(row.passed for row in rows) and bool(details["pass"])
    payload = {
        "schema": "qec_twin.simulator.axis1_g2_frontend_gate.v1",
        ...
        "bridge_scope": (
            "frontend G2 evidence only; no analog record emission, no Axis-2 source timeline, "
...
def write_axis1_g2_evidence(
    schedule: SubstepSchedule,
    out_dir: str | Path,
    *,
    device: str = "cuda",
    filename: str = "g2_jointL.json",
) -> Axis1G2EvidenceResult:
    """Write the build-contract G2 evidence object to `g2_jointL.json`.

    This writer intentionally produces only the G2 evidence artifact. It does
    ...
    _assert_clean_g2_evidence_dir(root)
    path = _safe_g2_evidence_output_path(root, filename)
    manifest = axis1_g2_gate_manifest(schedule, device=device)
    write_json(path, manifest)
    return Axis1G2EvidenceResult(
        out_dir=root,
        g2_jointL=path,
        manifest=manifest,
...
def freeze_axis1_g2_evidence(
    g2_jointL: str | Path,
    freeze_path: str | Path | None = None,
    *,
    overwrite: bool = False,
) -> Axis1G2FreezeResult:
    """Write a deterministic freeze guard for a `g2_jointL.json` artifact."""

    evidence_path = Path(g2_jointL)
    manifest = _load_g2_manifest(evidence_path)
    _validate_g2_manifest_content_hash(manifest, evidence_path)
    ...
    if out.exists() and not overwrite:
        ...
        if existing["evidence_file"] != evidence_path.name:
            raise ValueError(
                "Axis-1 G2 freeze evidence_file mismatch: "
                f"freeze={existing['evidence_file']!r} requested={evidence_path.name!r}"
            )
        validate_axis1_g2_freeze(out)
        return Axis1G2FreezeResult(
            freeze_path=out,
            evidence_path=evidence_path,
            manifest=existing,
        )
    payload = {
        "schema": "qec_twin.simulator.axis1_g2_freeze.v1",
        ...
        "scope": (
            "Axis-1 frontend G2 evidence freeze only; not a full analog simulator run, "
            "not Axis-2 source projection"
        ),
    }
    write_json(out, payload)
    return Axis1G2FreezeResult(freeze_path=out, evidence_path=evidence_path, manifest=payload)


def validate_axis1_g2_freeze(freeze_path: str | Path) -> dict:
    """Validate that the frozen `g2_jointL.json` evidence artifact did not drift."""
    ...
    if sha != freeze["evidence_sha256"]:
        raise ValueError(
            "Axis-1 G2 evidence sha256 mismatch: "
    ...
    if failed:
        raise ValueError(f"Axis-1 G2 freeze metadata mismatch: {failed}")
    return {
        "schema": "qec_twin.simulator.axis1_g2_freeze_validation.v1",
...
def _validate_schedule_for_g2(schedule: SubstepSchedule) -> None:
    if schedule.representability != ANALOG_SCHEDULE_REPRESENTABILITY:
        raise ValueError(
            "Axis-1 G2 frontend gate requires "
            f"{ANALOG_SCHEDULE_REPRESENTABILITY!r}, got {schedule.representability!r}"
        )
    if not schedule.source_hash:
        raise ValueError("Axis-1 G2 frontend gate requires a non-empty source_hash")
    if schedule.source_kind == "oracle_unit":
        raise ValueError("oracle_unit schedules are not accepted by the frontend G2 gate")
    if schedule.source_kind not in G2_ALLOWED_SOURCE_KINDS:
        allowed = ", ".join(sorted(G2_ALLOWED_SOURCE_KINDS))
        raise ValueError(
            "Axis-1 G2 frontend gate only accepts implemented schedule source kinds "
            f"({allowed}); got {schedule.source_kind!r}"
        )
    bad = [sub.substep_id for sub in schedule.substeps if not sub.generated_by_compiler]
    if bad:
        raise ValueError(f"Axis-1 G2 frontend gate requires compiler-generated substeps: {bad}")
...
def _dr_zz_nonzero_row(
    ...
    *,
    ...
    lo, hi = G2_DR_ZZ_BAND
    passed = (
        comm > G2_NONZERO_COMMUTATOR_MIN
        and superop_distance > G2_NONZERO_SUPEROP_DISTANCE_MIN
        and lo <= one_minus <= hi
    )
    failure = None
    if not passed:
...
def _axis1_g2_anti_toy_details(*, device: str) -> dict:
    """Full G2 anti-toy checks ported from the registered gate script.

    Rows carry schedule provenance. These details carry the old G2 gate's
    ...
    params = {
        ...
        "phys_band": list(G2_DR_ZZ_BAND),
        "fixed_omega_band": list(G2_FIXED_OMEGA_BAND),
        "dt_grid": list(G2_DT_GRID),
        "dt_control": list(G2_DT_CONTROL),
        "dt_small": list(G2_DT_SMALL),
        "fixed_omega_rad_per_ns": G2_FIXED_OMEGA_RAD_PER_NS,
        "slope_tol": G2_SLOPE_TOL,
    }
    return {
        "schema": "qec_twin.simulator.axis1_g2_anti_toy_details.v1",
...
def _check_dr_zz_band_and_powerlaws(*, device: str) -> dict:
    ...
    for dt in G2_DT_GRID:
        ...
        fixed_band_vals.append(
            composed_vs_joint_infidelity(
                [_H_DR(G2_FIXED_OMEGA_RAD_PER_NS, device=device), _H_ZZ(device=device)],
                c_list,
                dt,
                device=device,
            )
        )
    phys_in_band = all(G2_DR_ZZ_BAND[0] <= v <= G2_DR_ZZ_BAND[1] for v in phys_band_vals)
    fixed_in_band = all(G2_FIXED_OMEGA_BAND[0] <= v <= G2_FIXED_OMEGA_BAND[1] for v in fixed_band_vals)
    phys_full_context_slope = _loglog_slope(G2_DT_GRID, phys_band_vals)
    fixed_full_context_slope = _loglog_slope(G2_DT_GRID, fixed_band_vals)
    ...
    phys_slope = _loglog_slope(G2_DT_GRID, phys_ham_vals)
    phys_slope_ok = abs(phys_slope - 2.0) <= G2_SLOPE_TOL
    ...
    fixed_small_vals = [
        ...
        for dt in G2_DT_SMALL
    ]
    fixed_small_slope = _loglog_slope(G2_DT_SMALL, fixed_small_vals)
    fixed_small_slope_ok = abs(fixed_small_slope - 4.0) <= G2_SLOPE_TOL
    ...
    fixed_device_slope = _loglog_slope(G2_DT_GRID, fixed_device_vals)
    fixed_leading_slope = _loglog_slope(G2_DT_GRID, fixed_leading_vals)

    dt0 = 25.0
    omega0 = math.pi / dt0
    zeta_grid = [0.5 * G2_ZETA_RAD_PER_NS, G2_ZETA_RAD_PER_NS, 2.0 * G2_ZETA_RAD_PER_NS]
    zeta_vals = [
        composed_vs_joint_infidelity(
    ...
    zeta_slope_ok = abs(zeta_slope - 2.0) <= G2_SLOPE_TOL
    ...
    return {
        ...
        "fixed_omega": {
            "dt_grid_device": list(G2_DT_GRID),
            "dt_grid_small": list(G2_DT_SMALL),
            "band_vals_full_substep": fixed_band_vals,
            "band": list(G2_FIXED_OMEGA_BAND),
            ...
            "omega_const": G2_FIXED_OMEGA_RAD_PER_NS,
        },
        "zeta_scaling": {
            "zeta_grid": zeta_grid,
            "vals": zeta_vals,
...
def _H_ZZ(zeta: float = G2_ZETA_RAD_PER_NS, *, device: str):
    ...
    *,
    ...
    return default_axis1_primitive_registry().lower(
        ...
        params=Axis1PrimitiveParams(
            zeta_rad_per_ns=G2_ZETA_RAD_PER_NS if zeta_rad_per_ns is None else float(zeta_rad_per_ns),
            gamma_phi_per_ns=G2_GAMMA_PHI_PER_NS,
            gamma_1_per_ns=G2_GAMMA_1_PER_NS,
...
def _assert_clean_g2_evidence_dir(root: Path) -> None:
    forbidden = sorted(
        path.name
        for path in root.iterdir()
        if path.is_file()
        and _g2_forbidden_artifact_path(path)
    )
    if forbidden:
        raise ValueError(
            "Axis-1 G2 evidence writer refuses simulator-run artifact directories: "
            f"{forbidden}"
        )


def _safe_g2_evidence_output_path(root: Path, filename: str) -> Path:
    raw = str(filename)
    candidate = Path(raw)
    if not raw or candidate.is_absolute() or candidate.name != raw:
        raise ValueError(f"Axis-1 G2 evidence writer refuses non-local evidence filename: {raw!r}")
    if _g2_forbidden_artifact_path(candidate):
        raise ValueError(f"Axis-1 G2 evidence writer refuses forbidden evidence filename: {raw!r}")
    return root / candidate.name


def _g2_forbidden_artifact_path(path: Path) -> bool:
    suffixes = {str(s).lower() for s in G2_FORBIDDEN_OUTPUT_SUFFIXES}
    names = {str(name).lower() for name in G2_FORBIDDEN_OUTPUT_NAMES}
    return path.suffix.lower() in suffixes or path.name.lower() in names


def _load_g2_manifest(path: Path) -> dict:
    if not path.exists():
        raise FileNotFoundError(path)
    payload = json.loads(path.read_text())
    if not isinstance(payload, dict):
        raise ValueError(f"Axis-1 G2 evidence manifest must be a JSON object: {path}")
    if payload.get("schema") != "qec_twin.simulator.axis1_g2_frontend_gate.v1":
        raise ValueError(f"unexpected Axis-1 G2 evidence schema in {path}: {payload.get('schema')!r}")
    return payload
...
def _load_freeze_manifest(path: Path) -> dict:
    ...
    payload = json.loads(path.read_text())
    if not isinstance(payload, dict):
        raise ValueError(f"Axis-1 G2 freeze manifest must be a JSON object: {path}")
    if payload.get("schema") != "qec_twin.simulator.axis1_g2_freeze.v1":
        raise ValueError(f"unexpected Axis-1 G2 freeze schema in {path}: {payload.get('schema')!r}")
    required = {
        "evidence_file",
        "evidence_schema",
        "evidence_sha256",
    ...
    if missing:
        raise ValueError(f"Axis-1 G2 freeze manifest missing fields: {missing}")
    evidence_file = Path(str(payload["evidence_file"]))
    if evidence_file.name != str(payload["evidence_file"]) or evidence_file.is_absolute():
        raise ValueError("Axis-1 G2 freeze evidence_file must be a local filename")
    return payload


def _validate_g2_manifest_content_hash(manifest: dict, path: Path) -> None:
    expected = manifest.get("content_hash")
    if not isinstance(expected, str) or len(expected) != 64:
        raise ValueError(f"Axis-1 G2 evidence manifest has invalid content_hash: {path}")
    actual = _stable_payload_hash(manifest)
    if actual != expected:
        raise ValueError(
            "Axis-1 G2 evidence content_hash mismatch: "
