...
class StimNoiseRule:
    ...
    def __post_init__(self) -> None:
        ...
        if self.match_kind == "gate_index":
            if self.gate_index is None or int(self.gate_index) < 0:
                raise ValueError("gate_index rules require a non-negative gate_index")
            object.__setattr__(self, "gate_index", int(self.gate_index))
...
FrontendNoiseSpec = StimPauliNoiseSpec | TargetedStimNoiseSpec
...
def apply_stim_pauli_noise(circuit: CircuitIR, noise: FrontendNoiseSpec | None) -> CircuitIR:
    ...
    return CircuitIR(num_qubits=circuit.num_qubits, steps=tuple(steps), metadata=metadata)
