...
class CatalogPipelinePaths:
    ...
    def as_dict(self) -> dict[str, str]:
        return {
            ...
            "pipeline_json": str(self.pipeline_json),
            "pipeline_markdown": str(self.pipeline_markdown),
        }


def catalog_pipeline_paths(output_dir: str | Path) -> CatalogPipelinePaths:
    root = Path(output_dir)
    return CatalogPipelinePaths(
        root=root,
        ...
        pipeline_json=root / "catalog_pipeline.json",
        pipeline_markdown=root / "catalog_pipeline.md",
    )
...
def run_catalog_pipeline(
    ...
    *,
    ...
    cfg = dict(config or {})
    paths = catalog_pipeline_paths(output_dir)
    ...
    if mode == "always" or teacher_self_passed:
        start = time.perf_counter()
        local = _run_mechanism_observability_recovery(
            teacher_dir=paths.teacher_dir,
    ...
    result: dict[str, object] = {
        "schema": "scope_static_catalog_pipeline_v1",
        "stage": "Catalog Pipeline",
    ...
    paths.pipeline_json.write_text(json.dumps(result, indent=2, sort_keys=True, default=_json_default) + "\n")
    paths.pipeline_markdown.write_text(format_catalog_pipeline_summary(result))
    return result


def pipeline_stage_results(pipeline: dict[str, object]) -> dict[str, object]:
    paths = dict(pipeline.get("paths", {}))
    stages = dict(pipeline.get("stage_results", {}))
    return {
...
def format_catalog_pipeline_summary(result: dict[str, object]) -> str:
...
def _run_mechanism_observability_recovery(**kwargs):
    from scope_static.mechanism_observability import run_physical_local_inverse_discovery
...
def _compact_teacher(teacher: dict[str, object], paths: CatalogPipelinePaths) -> dict[str, object]:
    return {
