...
from scope_static.catalog_pipeline import run_catalog_pipeline
...
def run_s2d_difficulty_expansion(
    ...
    *,
    ...
    result = {
        ...
        "stage": "S2D.4_catalog_validation_difficulty_expansion",
...
def _run_one(output: Path, physical_cfg: dict[str, object], cfg: dict[str, object], run_cfg: dict[str, object]) -> dict[str, object]:
    ...
    teacher = dict(pipeline["teacher"])
    separability = dict(pipeline["metrics"]["teacher_self"])  # type: ignore[index]
    local = pipeline["metrics"].get("learner") if isinstance(pipeline.get("metrics"), dict) else None
    decision = str(pipeline["verdicts"]["overall_diagnosis"])  # type: ignore[index]
    ...
    record = {
        ...
        "catalog_pipeline": {
            ...
            "timings_seconds": pipeline.get("timings_seconds"),
        },
    }
...
def format_difficulty_summary(result: dict[str, object]) -> str:
    lines = [
        "# S2D.4 Catalog Validation Difficulty Expansion",
        "",
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Run S2D.4 catalog-validation difficulty expansion.")
