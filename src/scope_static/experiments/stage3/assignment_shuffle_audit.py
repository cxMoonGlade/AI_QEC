...
from scope_static.mechanism_discovery.assignment_shuffle_audit import (
...
def run_stage3d1_assignment_shuffle_audit_from_config(
    *,
    ...
    print(
        "Stage 3D.1 assignment-shuffle audit complete\n"
        f"  decision={result.get('decision')}\n"
        f"  original_categorical_population_nll={collapse.get('original_assignment')}\n"
        f"  shuffled_mean_categorical_population_nll={collapse.get('shuffled_assignment_mean')}\n"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Run Stage 3D.1 assignment-shuffle generator audit.")
    parser.add_argument("--config", type=Path, default=Path("configs/scope_static/stage3d1_assignment_shuffle_audit.yaml"))
...
def _load_config(config_path: str | Path | None) -> dict[str, object]:
    ...
    if not isinstance(data, dict):
        raise ValueError("Stage 3D.1 config must be a mapping")
    section = data.get("stage3d1_assignment_shuffle_audit", data)
    if not isinstance(section, dict):
        raise ValueError("Stage 3D.1 config section must be a mapping")
    return dict(section)
