...
def run_phyc3_canonical_acceptance_from_config(
    *,
    config_path: str | Path | None = None,
    output_dir: str | Path | None = None,
) -> dict[str, object]:
    cfg = _load_config(config_path)
    output = Path(output_dir) if output_dir is not None else Path(str(cfg.get("output_dir", "outputs/scope_static/PHYC3_canonical_quality_acceptance")))
...
def main(argv: list[str] | None = None) -> None:
    ...
    run_phyc3_canonical_acceptance_from_config(config_path=args.config, output_dir=args.output_dir)
