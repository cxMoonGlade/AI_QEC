...
def load_stage4_source_evaluator_labels(stage4_source_dir: str | Path) -> Stage4SourceLabels:
    ...
    payload = load_json_object(source / "source_evaluator_labels.json")
    records_raw = payload.get("records", [])
    if not isinstance(records_raw, list) or not records_raw:
        raise ValueError(f"{source / 'source_evaluator_labels.json'} must contain non-empty evaluator records")
